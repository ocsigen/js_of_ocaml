...
(* -- FIXME -- [merlin_noloc] is an attempt to hide some ast node from merlin
   when using merlin-type-enclosing-go-{up,down}. It turns out not to be
   working properly, just hiding location instead of the all ast node. *)
...
(* uplift : type of the unused value - ties all types together downlift : types
   of individual components (arguments and result) *)
let invoker ?(extra_types = []) uplift downlift body arguments =
  ...
  (* Build the function. The last arguments is just used to tie all types
     together. It's unused in the implementation. {[ fun (t1 : type_of_t1) (t2
     : type_of_t2) (_ : uplift_type) -> e]} *)
  let labels_and_pats =
    List.map
      (fun d ->
...
(* {[ obj##meth x y ]} generates {[ ( fun (type res a2 a0 a1) -> fun (a2 : a2
   Js.t) -> fun (a0 : a0) -> fun (a1 : a1) -> fun (_ : a2 -> a0 -> a1 -> res
   Js.meth) -> (Js.Unsafe.meth_call a2 "meth" [|(Js.Unsafe.inject a0);
   (Js.Unsafe.inject a1) |] : res) ) (obj : < .. > Js.t) x y (fun x -> x#meth)
   ]} *)
...
(* {[ obj##.prop ]} generates {[ ( fun (type res a0) -> fun (a0 : a0 Js.t) ->
   fun (_ : a0 -> < get :res ;.. > Js.gen_prop) -> (Js.Unsafe.get a0 "prop" :
   res) ) (obj : < .. > Js.t) (fun x -> x#prop) ]} *)
...
(* {[ obj##.prop := expr ]} generates {[ ( fun (type res a1 a0) -> fun (a1 : a1
   Js.t) -> fun (a0 : a0) -> fun (_ : a1 -> < set :a0 -> unit ;.. >
...
(* {[ new%js constr x y ]} generates {[ ( fun (type res a2 a0 a1) -> fun (a2 :
   (a0 -> a1 -> res Js.t) Js.constr) -> fun (a0 : a0) -> fun (a1 : a1) -> fun
   (_ : unit) -> (Js.Unsafe.new_obj a2 [|(Js.Unsafe.inject a0);
...
(** We remove Pexp_poly as it should never be in the parsetree except after a
    method call. *)
let format_meth body =
  match body.pexp_desc with Pexp_poly (e, _) -> e | _ -> body

(** Ensure basic sanity rules about fields of a literal object: - No duplicated
    declaration - Only relevant declarations (val and method, for now). *)
...
let preprocess_literal_object mappper fields :
  ...
  let check_name id names =
    ...
    then
      ...
      in
      let sub =
        [ Location.errorf ~loc:id'.loc "Duplicated val or method %S%s." id'.txt
            (details id') ]
      in
      Location.raise_errorf ~loc:id.loc ~sub "Duplicated val or method %S%s."
        id.txt (details id)
...
(* {[ object%js (self) val readonlyprop = e1 val prop = e2 method meth x = e3
   end ]} generates {[ ( fun (type res a6 a7 a8 a9) -> fun (a7 : a7) -> fun (a8
   : a8) -> fun (a9 : res Js.t -> a6 -> a9) -> fun (_ : res Js.t -> a7
   Js.readonly_prop -> a8 Js.prop -> (res Js.t -> a6 -> a9 Js.meth) -> res) ->
   (Js.Unsafe.obj [|("readonlyprop", (Js.Unsafe.inject a7)); ("prop",
   (Js.Unsafe.inject a8)); ("meth", (Js.Unsafe.inject (Js.wrap_meth_callback
   a9))) |] : res Js.t) ) e1 e2 (fun self -> fun x -> e3) (fun self
   read_only_prop prop meth -> object method read_only_prop = read_only_prop
   method prop = prop method meth = meth self end) ]} *)
let literal_object self_id (fields : field_desc list) =
  ...
  let invoker =
    invoker ~extra_types
      ...
      (List.map
         (function
           | Val _ -> Arg.make () | Meth (_, _, _, _, _fun_ty) -> Arg.make ())
         fields)
  in
  let self = "self" in
          ...
          List.map
            (fun f ->
              ...
              let apply e =
                ...
                | Meth _ ->
                    Exp.apply e
                      [Label.nolabel, Exp.ident (lid ~loc:Location.none self)]
              in
              { pcf_loc= loc
              ; pcf_attributes= []
              ; pcf_desc=
    ...
    expr=
      (fun mapper expr ->
        ...
        let new_expr =
          ...
          (* obj##.var := value *)
          | [%expr
              [%e? [%expr [%e? obj] ##. [%e? meth]] as prop] := [%e? value]] ->
              let obj = mapper.expr mapper obj in
          ...
          (* obj##meth arg1 arg2 .. *)
          | { pexp_desc=
                Pexp_apply (([%expr [%e? obj] ## [%e? meth]] as prop), args); _
            } ->
              let meth_str = exp_to_string meth in
              let obj = mapper.expr mapper obj in
              let args =
          ...
          | { pexp_desc=
                Pexp_apply
                  ([%expr [%js [%e? {pexp_desc= Pexp_new constr; _}]]], args); _
            } ->
              let args =
                List.map (fun (s, e) -> s, mapper.expr mapper e) args
              in
