...
(* - stop animation when not needed ==> not visible ==> no change (paused,
   follow rotation and no lighting) - Options: ==> larger/smaller - adaptative
   size ==> time 3 frames and take min ==> if fast, try larger image

   IDEAS ===== - saisons - satellites: geostationnaires, différentes altitudes
   ==> trajectoire + mouvement du satellite - affiche l'axe de rotation de la
   terre, la direction du soleil - autres planètes
   ...
   http://visibleearth.nasa.gov/view_rec.php?id=2431 http://maps.jpl.nasa.gov/ *)
open Js_of_ocaml
...
let radio txt name checked action =
  let b =
    Dom_html.createInput ~name:(Js.string name) ~_type:(Js.string "radio") doc
  in
  b##.checked := Js.bool checked ;
  b##.onclick := Dom_html.handler (fun _ -> action () ; Js._true) ;
  let lab = Dom_html.createLabel doc in
    ...
    (fun i {v1; v2; v3} ->
      ...
      then (
        ...
        (* let (u1, v1) = uv.(v1) in let (u2, v2) = uv.(v2) in let (u3, v3) =
           ...
           let u1 = if u1 = 0. && (u2 > mid || u3 > mid) then tw -. 2. else u1
           in let u2 = if u2 = 0. && (u1 > mid || u3 > mid) then tw -. 2. else
           u2 in let u3 = if u3 = 0. && (u2 > mid || u1 > mid) then tw -. 2.
           else u3 in
           ...
           let u' = min tw (max u1 (max u2 u3) +. 4.) in let v' = min th (max
           v1 (max v2 v3) +. 4.) in let du = u' -. u in let dv = v' -. v in *)
...
let start _ =
  Lwt.ignore_result
    ...
    ( add form (toggle_button "Pause" "Resume" (fun p -> paused := p)) ;
      add form (br ()) ;
      add form
        (toggle_button "Follow rotation" "Fixed position" (fun f -> follow := f)) ;
      add form (br ()) ;
      add form
