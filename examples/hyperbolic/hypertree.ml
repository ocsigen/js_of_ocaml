...
(* - check compatibility + wait page
   ...
   - we should stop updating the canvas when it is not visible (opaque overlay)
   ...
   - récupérer le nom des animaux? ==> overlay mechanism: save what is below,
   display overlay; restore when moved

   - can we have fast shadows? ==> precompute the shadows in a canvas? - should
   we have half size images for better rendering??? (might make it possible
   fast shadows?)
   ...
   - get all image sizes so that we can size precisely images before they are
   all loaded

   ==============

   - find point closest to center; start drawing from it; stop below a given
   threshold - speed-up rendering by traversing the tree and stopping when the
   ...
   - importante répulsion quand trop près! ==> surtout pour feuilles! - cap
   the acceleration
...
(* let d = Html.document in Dom.appendChild d##body debug_widget; let p =
   Html.createP d in p##innerHTML <- Js.string s; Dom.appendChild debug_widget
   p *)
        ...
        let c1 =
          Html.addEventListener Html.document Html.Event.mousemove
            (Html.handler (fun ev ->
                 ...
                 (* debug_msg (Format.sprintf "Mouse move %d %d %d %d" x0 y0 x
                    y); *)
                 if (not !started)
        ...
        c2 :=
          Js.some
            (Html.addEventListener Html.document Html.Event.mouseup
               (Html.handler (fun ev ->
                    (* debug_msg (Format.sprintf "Mouse up %d %d %d %d" x0 y0
                           ...
                           Js.Optdef.iter
                             ...
                             (fun touch ->
                               ...
                               then (
                                 ...
                                 (* debug_msg (Format.sprintf "Touch move %d %d
                                    %d %d" x0 y0 x y); *)
                                 if (not !started)
                                    && ( abs (x - x0) > fuzz
                                       || abs (y - y0) > fuzz )
                              ...
                              Js.Optdef.iter
                                ...
                                (fun touch ->
                                  ...
                                  then (
                                    let x = touch##.clientX
                                    and y = touch##.clientY in
                                    (* debug_msg (Format.sprintf "Touch end %d
                                       %d %d %d" x0 y0 x y); *)
                                    Html.removeEventListener c1 ;
                                    Js.Opt.iter !c2 Html.removeEventListener ;
                              ...
                              Js.Optdef.iter
                                ...
                                (fun touch ->
                                  ...
                                  then (
                                    let x = touch##.clientX
                                    and y = touch##.clientY in
                                    (* debug_msg (Format.sprintf "Touch cancel
                                       %d %d %d %d" x0 y0 x y); *)
                                    Html.removeEventListener c1 ;
                                    Js.Opt.iter !c2 Html.removeEventListener ;
                                    Js.Opt.iter !c3 Html.removeEventListener ;
...
(* let handle_touch_events element move stop cancel = let fuzz = 4 in let id =
   ref 0 in let x0 = ref 0 in let y0 = ref 0 in ignore (Html.addEventListener
   element Html.Event.touchstart (Html.handler (fun ev -> OptDef.iter
   (ev##changedTouches##item(0)) (fun touch -> id := touch##identifier; in x0
   := touch##clientX; y0 := touch##clientY)); (* x0 := ev##pageX; y0 :=
   ev##pageY;*) debug_msg (Format.sprintf "Touch down %d %d" !x0 !y0);
   Js._false)) Js._false); ignore (Html.addEventListener element
   Html.Event.touchmove (Html.handler (fun ev -> (* let x = ev##pageX and y =
   ev##pageY in *) debug_msg (Format.sprintf "Touch move %d %d %d %d" !x0 !y0 x
   y); move !x0 !y0 x y; Js._false)) Js._false); ignore (Html.addEventListener
   element Html.Event.touchend (Html.handler (fun _ -> debug_msg
   (Format.sprintf "Touch end"); stop (); Js._false)) Js._false); ignore
   (Html.addEventListener element Html.Event.touchcancel (Html.handler (fun _
   -> cancel (); Js._true)) Js._false) *)
...
(* We have |z0|^2 = 1 + r^2 (the "line" is orthogonal to the unit circle)
   |z1-z0|^2 = r^2 (z1 is on the circle) |z2-z0|^2 = r^2 (z2 is on the circle)
    ...
    | `Img (img, _) -> (
        ...
        then (
          ...
          | Some img ->
              ...
              (* let s = min_scale l w h (1. /. min w h) in let scale = s *. r
                 in *)
              let s = min_scale l 1. 1. 1. in
              ...
              if w > 1. && h > 1.
              then (
                (* let ratio = r *. min (min_w /. w) (min_h /. h) in let w = w
                   *. ratio in let h = h *. ratio in *)
                if shadow
                then (
                  c##save ;
                  (* let blur = 7. /. img_r *. rd in let offset = 5. /. img_r
                     *. rd in *)
                  let blur = 7. *. scale in
...
(* let load_image src = load_image src >>= fun img -> let w = img##width in let
   h = img##height in let canvas = create_canvas (w + 14) (h + 14) in let c =
   canvas##getContext (Html._2d_) in c##shadowBlur <- 7.; c##shadowOffsetX <-
   5.; c##shadowOffsetY <- 5.; c##shadowColor <- Js.string "black";
   c##drawImage_withSize (img, 2., 2., float w, float h); Lwt.return canvas *)
...
let information_en =
  Js.string
    "<h1>A tree of animals</h1> This <a target=\"_blank\" \
     ...
     tree</a> displays the relationships among animals.<h2>Usage</h2>Browse \
     the tree by dragging it with the mouse. Click on any image to display a \
     larger version.<h2>Credits</h2>This software and the images it uses are \
     under free licenses. Images are from <a target=\"_blank\" \
     href=\"http://commons.wikimedia.org/wiki/Main_Page\">Wikimedia \
     Commons</a>.  You can click on each image for details.  The software has \
     been written by Jérôme Vouillon (CNRS)."
  ...
  Lwt.ignore_result
    ...
    (redraw_funct :=
       fun () ->
         ...
         (* let transf = hyp_transf !tr' in for i = 0 to Array.length vertices
            - 1 do vertices'.(i) <- transf vertices.(i) done; *)
         let w = page##.clientWidth in
         let h = page##.clientHeight in
    ...
    (* let eventually t f = let scheduled = ref false in fun () -> if not
       !scheduled then begin scheduled := true; ignore (Lwt_js.sleep t >>= fun
       () -> scheduled := false; f (); Lwt.return ()) end in *)
    ...
    handle_drag canvas
      (fun x0 y0 x1 y1 ->
        ...
        (* (* Transformation from z0 to z1: z1 = (z0 + p) / (conj p.z0 + 1) ==>
           p = (z1.z0.conj (z1 - z0) + z1 - z0) / (1 - |z1.z0|^2) *) let dz =
           sub z1 z0 in let z0z1 = mul z0 z1 in let p = sdiv (add (mul z0z1
           (conj dz)) dz) (1. -. sq_norm z0z1) in tr' := compose !tr (p, one); *)
        let p, _ = !tr in
