...
let _ =
  let canvas = 
    Option.get
      @@ Dom_html.getElementById_coerce "main-canvas" Dom_html.CoerceTo.canvas in
  let context = canvas##getContext (Dom_html._2d_) in
  let bebas = Css_font.create_font_face
    (Js.string "Bebas Neue") (Js.string "url(BebasNeue-Regular.ttf)")
  in
  Css_font.load_and_then bebas (fun () ->
    is_loading := false
  );
