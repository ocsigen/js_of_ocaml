...
let parse_color c =
  ...
  then
    ...
    let c =
      conv (String.sub c 1 2), conv (String.sub c 3 2), conv (String.sub c 5 2)
    in
    Some (convert c)
  else
    Some
...
let rec read_element nm attrs i =
  ...
  | `El_start ((_, nm'), attrs') ->
      (* Format.eprintf "%s" nm'; List.iter (fun ((_, nm), _) -> Format.eprintf
         " %s" nm) attrs'; Format.eprintf "@."; *)
      ( match nm' with
...
let redraw w range ev =
  ...
  in
  (* let (x1, y1, x2, y2) = bbox in Format.eprintf "%f %f %f %f (%f)@." x1 y1
     x2 y2 scale; *)
  List.iter2
...
let _ =
  ...
  (* let button = GButton.check_button ~label:"Animate" ~packing:b#pack () in
...
(* let _ = let l = List.rev !stack in Format.eprintf "len: %d@." (List.length
   l); let t1 = Unix.gettimeofday () in List.iter (fun e -> draw_element ctx e)
   l; let t2 = Unix.gettimeofday () in Format.eprintf "%f@." (t2 -. t1);

   (* let ch = open_out "/tmp/foo.mar" in Marshal.to_channel ch l []; close_out
   ch; *) Cairo_png.surface_write_to_file s "/tmp/foo.png" *)
