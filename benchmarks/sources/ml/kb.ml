...
let mk_rule num m n =
  ...
  let subst =
    List.map (fun v -> incr counter ; v, Var !counter) (List.rev all_vars)
  in
  { number= num
  ; numvars= !counter
...
let rec mreduce rules m =
  ...
  | [] -> failwith "mreduce"
  | rule :: rest -> (
    try reduce rule.lhs m rule.rhs with Failure _ -> mreduce rest m )
...
and mrewrite1_sons rules = function
  | [] -> failwith "mrewrite1"
  | son :: rest -> (
    try mrewrite1 rules son :: rest with Failure _ ->
...
let mult_ext order = function
  | Term (_, sons1), Term (_, sons2) -> (
    ...
    | l1, l2 ->
        if List.for_all
             (fun n -> List.exists (fun m -> gt_ord order (m, n)) l1)
             l2
        then Greater
        else NotGE )
  | _ -> failwith "mult_ext"
  ...
  | (Term (_, sons1) as m), (Term (_, sons2) as n) ->
      let rec lexrec = function
        ...
        | x1 :: l1, x2 :: l2 -> (
          ...
          | Equal -> lexrec (l1, l2)
          | NotGE ->
              if List.exists (fun m' -> ge_ord order (m', n)) l1
              then Greater
              else NotGE )
      in
      lexrec (sons1, sons2)
...
(* All (u,subst) such that N/u (&var) unifies with M, with principal unifier
   subst *)
...
(* Ex : let (m,_) = <<F(A,B)>> and (n,_) = <<H(F(A,x),F(x,y))>> in super m n
   ==> [[1],[2,Term ("B",[])]; x <- B [2],[2,Term ("A",[]); 1,Term ("B",[])]] x
   <- A y <- B *)
...
let critical_pairs (l1, r1) (l2, r2) =
  let mk_pair (u, subst) =
    substitute subst (replace l2 u r1), substitute subst r2
  in
  ...
  let rec kbrec j rules =
    let rec process failures (k, l) eqs =
      (**** print_string "***kb_completion "; print_int j; print_newline();
        pretty_rules rules; List.iter non_orientable failures; print_int k;
        print_string " "; print_int l; print_newline(); List.iter
        non_orientable eqs; ***)
      match eqs with
      | [] -> (
          if k < l
          ...
          and enter_rule (left, right) =
            ...
            let right_reduce rule =
              mk_rule rule.number rule.lhs
                (mrewrite_all (new_rule :: rules) rule.rhs)
            in
    ...
    and next_criticals failures (k, l) =
      (**** print_string "***next_criticals "; print_int k; print_string " ";
...
(**** let group_rules = [ { number = 1; numvars = 1; lhs = Term("*",
  [Term("U",[]); Var 1]); rhs = Var 1 }; { number = 2; numvars = 1; lhs =
  Term("*", [Term("I",[Var 1]); Var 1]); rhs = Term("U",[]) }; { number = 3;
  numvars = 3; lhs = Term("*", [Term("*", [Var 1; Var 2]); Var 3]); rhs =
