module Color = struct
  ...
  (* let print ch c = let r = truncate (c.red *. 255.) in let g = truncate
     (c.green *. 255.) in let b = truncate (c.blue *. 255.) in Format.fprintf
     ch "rgb(%d,%d,%d)" r g b *)
          ...
          then None
            ...
            Some
              ...
              ; color=
                  ...
                  then
                    let vu =
                      Vector.make position.Vector.y position.Vector.z
                        (-.position.Vector.x)
                    in
...
module Engine = struct
  ...
  let rec ray_trace options info ray scene depth =
    let old_color =
      Color.multiply_scalar info.Intersection_info.color
    ...
    for i = 0 to Array.length lights - 1 do
      ...
      then
        ...
        then
          let reflection_ray =
            get_reflection_ray info.Intersection_info.position
              info.Intersection_info.normal ray.Ray.direction
          in
          let col =
            match
              test_intersection reflection_ray scene
      ...
      then (
        ...
        | Some info ->
            ...
            let db =
              0.5
              *. info.Intersection_info.shape.Shape.material
                   .Material.transparency ** 0.5
            in
            color := Color.add_scalar va db
        | None -> () ) ;
      if options.render_highlights && !shadow_info <> None
      ...
      then
        ...
        color :=
          Color.add
            (Color.multiply_scalar light.Light.color gloss_weight)
            !color
    done ;
    Color.limit !color
  ...
  let set_pixel options x y color =
    ...
    ( (* let pxw = options.pixel_width in let pxh = options.pixel_height in
         Format.eprintf "%d %d %d %d %d %a@." (x * pxw) (y * pxh) pxw pxh
         !check_number Color.print color; *) )
  ...
  let make canvas_width canvas_height pixel_width pixel_height render_diffuse
...
let render_scene () =
  let camera =
    Camera.make (Vector.make 0. 0. (-15.)) (Vector.make (-0.2) 0. 5.)
      (Vector.make 0. 1. 0.)
  in
  ...
  let plane =
    Shape.make
      ...
      (Material.chessboard (Color.make 1. 1. 1.) (Color.make 0. 0. 0.) 0.2 0.
         1.0 0.7)
  in
  let light =
    Light.make (Vector.make 5. 10. (-1.)) (Color.make 0.8 0.8 0.8) 10.
  in
  let light1 =
    Light.make (Vector.make (-3.) 5. (-15.)) (Color.make 0.8 0.8 0.8) 100.
  in
  let scene =
  ...
  let ray_depth = 2 in
  let engine =
    Engine.make image_width image_height (fst pixel_size) (snd pixel_size)
