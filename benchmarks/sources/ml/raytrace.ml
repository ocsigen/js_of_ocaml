...
*)
  ...
  let multiply_vector v w =
    { x = v.x *. w.x; y = v.y *. w.y; z = v.z *. w.z }

  let multiply_scalar v w =
...
module Light = struct
  ...
  type t = { position : Vector.t; color : Color.t; intensity : float }
...
module Ray = struct
  ...
  type t = { position : Vector.t; direction : Vector.t }
...
module Camera = struct
  ...
  type t =
    { position : Vector.t; look_at : Vector.t;
      equator : Vector.t; up : Vector.t; screen : Vector.t }
...
module Background = struct
  ...
  type t = { color : Color.t; ambience : float }
...
module Material = struct
  ...
  type t =
    { reflection : float;
      transparency : float;
      gloss : float;
      has_texture : bool; get_color : float -> float -> Color.t }
  ...
  let solid color reflection transparency gloss =
    { reflection = reflection; transparency = transparency;
      gloss = gloss; has_texture = false; get_color = fun _ _ -> color }

  let chessboard color_even color_odd reflection transparency gloss density =
    { reflection = reflection; transparency = transparency;
      gloss = gloss; has_texture = true;
      get_color =
        fun u v ->
...
module Shape = struct
  ...
  let make shape material = { shape = shape; material = material }

  let dummy =
    make (Sphere (Vector.make 0. 0. 0., 0.))
         (Material.solid (Color.make 0. 0. 0.) 0. 0. 0.)

  let position s =
    match s.shape with Sphere (p, _) -> p | Plane (p, _) -> p
  ...
  let intersect s ray =
    match s.shape with
      Sphere (position, radius) ->
        ...
        if d > 0. then begin
          ...
          let pos =
            Vector.add ray.Ray.position
              (Vector.multiply_scalar ray.Ray.direction dist)
          in
          Some { Intersection_info.shape = s;
                 distance = dist;
                 position = pos;
    ...
    | Plane (position, d) ->
      ...
      if vd = 0. then None else begin
        let t = -. (Vector.dot position ray.Ray.position +. d) /. vd in
        if t <= 0. then None else begin
          let pos =
            Vector.add ray.Ray.position
              (Vector.multiply_scalar ray.Ray.direction t)
          in
          Some { Intersection_info.shape = s;
                 distance = t;
                 position = pos;
                 normal = position;
                 color =
                   if s.material.Material.has_texture then begin
                     let vu = Vector.make position.Vector.y
                                          position.Vector.z
                                          (-. position.Vector.x)
                     in
                     ...
                     s.material.Material.get_color 0. 0. }
        end
      end
end
...
module Scene = struct
  ...
  let make c s l b =
    { camera = c; shapes = s; lights = l; background = b }
...
module Engine = struct
  ...
  let rec ray_trace options info ray scene depth =
    let old_color =
      Color.multiply_scalar
        info.Intersection_info.color scene.Scene.background.Background.ambience
    in
    let color = ref old_color in
    let shininess =
      10. **
      (info.Intersection_info.shape.Shape.material.Material.gloss +. 1.)
    in
    ...
    for i = 0 to Array.length lights - 1 do
      ...
      let v =
        Vector.normalize
          (Vector.subtract
             light.Light.position info.Intersection_info.position)
      in
      if options.render_diffuse then begin
        let l = Vector.dot v info.Intersection_info.normal in
        if l > 0. then
          color :=
            Color.add !color
              (Color.multiply info.Intersection_info.color
                 (Color.multiply_scalar light.Light.color l))
      end;
      if depth <= options.ray_depth then begin
        if
      ...
      if options.render_shadows then begin
        let shadow_ray = Ray.make info.Intersection_info.position v in
        shadow_info :=
          test_intersection shadow_ray scene info.Intersection_info.shape;
        match !shadow_info with
          Some info ->
            ...
            let db =
              0.5 *.
                info.Intersection_info.shape
                    .Shape.material.Material.transparency ** 0.5
            in
      ...
      if
        options.render_highlights && !shadow_info <> None &&
      ...
      then begin
        ...
        let lv =
          Vector.normalize
            (Vector.subtract shape_position light.Light.position)
        in
        let e =
          Vector.normalize
        ...
        let gloss_weight =
          (max (Vector.dot info.Intersection_info.normal h) 0.) **
          shininess
        in
        color :=
          Color.add
            (Color.multiply_scalar light.Light.color gloss_weight) !color
      end
    done;
    Color.limit !color
  ...
  and test_intersection ray scene exclude =
    ...
    for i = 0 to Array.length shapes - 1 do
      ...
      if shape != exclude then begin
        match Shape.intersect shape ray with
          Some { Intersection_info.distance = d } as v
              when d >= 0. && d < !dist ->
            best := v;
            dist := d
        | _ ->
...
*))
  ...
  let make
        canvas_width canvas_height pixel_width pixel_height
        render_diffuse render_shadows render_highlights render_reflections
        ray_depth =
    { canvas_width = canvas_width / pixel_width;
      ...
      render_reflections = render_reflections;
      ray_depth = ray_depth }
...
let render_scene () =
  ...
  let plane =
    Shape.make
      ...
      (Material.chessboard (Color.make 1. 1. 1.) (Color.make 0. 0. 0.)
         0.2 0. 1.0 0.7)
  in
  let light =
    Light.make (Vector.make 5. 10. (-1.)) (Color.make 0.8 0.8 0.8) 10. in
  let light1 =
    Light.make (Vector.make (-3.) 5. (-15.)) (Color.make 0.8 0.8 0.8) 100. in
  let scene =
  ...
  let engine =
    Engine.make
      image_width image_height (fst pixel_size) (snd pixel_size)
      render_diffuse render_shadows render_highlights render_reflections
      ray_depth
  in
