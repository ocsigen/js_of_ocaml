...
(* The notation for the transformations follows "Paul, R.P. (1981) Robot
   ...
   matrices don't have the perspective terms and are the transpose of Paul's
   one. See also "M\"antyl\"a, M. (1985) An Introduction to Solid Modeling,
   Computer Science Press" Appendix A.
...
(* The function "tfo-apply" multiplies a transformation matrix, tfo, by a point
   vector, p. The result is a new point. *)
...
(* The function "tfo-combine" multiplies two transformation matrices A and B.
   The result is a new matrix which cumulates the transformations described by
   A and B. *)

let tfo_combine a b =
  (* <HAND_CSE> *)
  (* Hand elimination of common subexpressions. Assumes lots of float registers
     (32 is perfect, 16 still OK). Loses on the I386, of course. *)
  let a_a = a.a
  and a_b = a.b
  and a_c = a.c
  and a_d = a.d
...
(* <NO_CSE> *)
(*** { a = ((a.a * b.a) + (a.b * b.d) + (a.c * b.g)); b = ((a.a * b.b) + (a.b *
  b.e) + (a.c * b.h)); c = ((a.a * b.c) + (a.b * b.f) + (a.c * b.i)); d = ((a.d
  * b.a) + (a.e * b.d) + (a.f * b.g)); e = ((a.d * b.b) + (a.e * b.e) + (a.f *
  b.h)); f = ((a.d * b.c) + (a.e * b.f) + (a.f * b.i)); g = ((a.g * b.a) + (a.h
  * b.d) + (a.i * b.g)); h = ((a.g * b.b) + (a.h * b.e) + (a.i * b.h)); i =
  ((a.g * b.c) + (a.h * b.f) + (a.i * b.i)); tx = ((a.tx * b.a) + (a.ty * b.d)
  + (a.tz * b.g) + b.tx); ty = ((a.tx * b.b) + (a.ty * b.e) + (a.tz * b.h) +
  b.ty); tz = ((a.tx * b.c) + (a.ty * b.f) + (a.tz * b.i) + b.tz) } ***)
(* </NO_CSE> *)
...
(* Given three points p1, p2, and p3, the function "tfo-align" computes a
   transformation matrix such that point p1 gets mapped to (0,0,0), p2 gets
...
(* A n6 n7 n9 c8 h2 h61 h62 h8 C n4 o2 h41 h42 h5 h6 G n2 n7 n9 c8 o6 h1 h21
   h22 h8 U o2 o4 h3 h5 h6 *)
...
(* dgf_base_tfo ; defines the standard position for wc and wc_dumas
   ...
   p_o3'_180_tfo p_o3'_60_tfo p o1p o2p o5' c5' h5' h5'' c4' h4' o4' c1' h1'
   c2' h2'' o2' h2' c3' h3' o3' n1 n3 c2 c4 c5 c6 *)
...
(* Primary structure: strand A CUGCCACGUCUG, strand B CAGACGUGGCAG
   ...
   Secondary structure: strand A CUGCCACGUCUG |||||||||||| GACGGUGCAGAC strand
   B

   Tertiary structure:

   5' end of strand A C1----G12 3' end of strand B U2-------A11 G3-------C10
   C4-----G9 C5---G8 A6 G6-C7 C5----G8 A4-------U9 G3--------C10 A2-------U11
   5' end of strand B C1----G12 3' end of strand A

   "helix", "stacked" and "connected" describe the spatial relationship between
   two consecutive nucleotides. E.g. the nucleotides C1 and U2 from the strand
   A.

   "wc" (stands for Watson-Crick and is a type of base-pairing), and "wc-dumas"
   describe the spatial relationship between nucleotides from two chains that
   are growing in opposite directions. E.g. the nucleotides C1 from strand A
   and G12 from strand B. *)

(* Dynamic Domains *)

(* Given, "refnuc" a nucleotide which is already positioned, "nucl" the
   nucleotide to be placed, and "tfo" a transformation matrix which expresses
   the desired relationship between "refnuc" and "nucl", the function
   "dgf-base" computes the transformation matrix that places the nucleotide
   "nucl" in the given relationship to "refnuc". *)
...
(* The transformation matrix for wc is from:
   ...
   Chandrasekaran R. et al (1989) A Re-Examination of the Crystal Structure of
   A-DNA Using Fiber Diffraction Data. J. Biomol. Struct. & Dynamics
   6(6):1189-1202. *)
