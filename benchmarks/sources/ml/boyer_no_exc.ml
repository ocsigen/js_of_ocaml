...
let apply_subst alist term =
  let rec as_rec = function
     Var v ->
       begin match get_binding v alist with Some t -> t | None -> term end
...
and unify1 term1 term2 unify_subst =
 match term2 with
    Var v ->
      begin match get_binding v unify_subst with
        Some t when t = term1 ->
          Some unify_subst
      | Some _ ->
          None
      | None ->
          Some (Bind(v,term1) :: unify_subst)
      end
  | Prop (head2, argl2) ->
      match term1 with
         Var _ -> None
...
and unify1_lst l1 l2 unify_subst =
  match (l1, l2) with
    ([], []) -> Some unify_subst
  | (h1::r1, h2::r2) ->
    begin match unify1 h1 h2 unify_subst with
...
and rewrite_with_lemmas term lemmas =
  ...
  | (t1,t2)::rest ->
      match unify term t1 with
        Some unify_subst ->
          rewrite (apply_subst unify_subst t2)
      | None ->
