...
# the benchmarks when compiled with wasm_of_ocaml, at --opt 1 and 2, with
# the LCM pass enabled and disabled.
#
# Usage: count.sh OUTDIR
#
# Writes OUTDIR/results.txt, one line per program, --opt level and mode
# ("lcm" / "nolcm") followed by the counts, then prints a summary table.
...
# JOBS (parallel jobs, default 8). Binaryen and node must be in PATH.
set -e
