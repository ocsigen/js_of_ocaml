(*
Iterate over closures;
Get bound vars in closure
Get free vars in closure
Take difference
*)
...
let closure_free_vars = ref (fun params (pc, _) -> assert false)

let cont_free_vars b (_, l) s = list_fold add_var b l s

let expr_free_vars b x e s =
  match e with
  ...
  | Code.Closure (params, cont) ->
      let s' = !closure_free_vars params cont in
(*
...
let instr_free_vars b i s =
  match i with
    Code.Let (x, e) ->
      expr_free_vars b x e s
...
let block_free_vars b (params, instr, last) s =
  s >> list_fold instr_free_vars b instr >> last_free_var b last
...
let rec traverse pc visited blocks bound_vars free_vars =
  if not (Util.IntSet.mem pc visited) then begin
    ...
    let free_vars = block_free_vars bound_vars block free_vars in
...
let f (pc, blocks, free_pc) =
  ...
  closure_free_vars :=
    (fun params (pc, _) ->
       ...
       ctx := Util.IntMap.add pc free_vars !ctx;
       free_vars);
  let free_vars = !closure_free_vars [] (pc, []) in
(*
