...
When OCaml code calls `Sys.getenv`, variables are resolved in this order:
...
4. Raise `Not_found`
With wasm\_of\_ocaml targeting WASI, lookups fall back to the environment provided by the WASI host. The Node.js launcher builds this environment from `process.env` and `globalThis.jsoo_env`, the former taking precedence.
