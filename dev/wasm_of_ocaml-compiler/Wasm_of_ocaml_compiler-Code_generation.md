...
type context = {
  ...
  string_globals : Js_of_ocaml_compiler.Code.Var.t
  ...
  mutable cps_dummy_funs : Js_of_ocaml_compiler.Code.Var.t
                           Js_of_ocaml_compiler.Stdlib.IntMap.t;
  mutable init_code : Wasm_ast.instruction list; (* Initialization code, stored in reverse execution order *)
...
val intern_string : 
  string ->
  (unit -> Js_of_ocaml_compiler.Code.Var.t t) ->
  Js_of_ocaml_compiler.Code.Var.t t
```
`intern_string s gen` returns the global holding the string constant `s`, calling `gen` to create it the first time `s` is encountered.
