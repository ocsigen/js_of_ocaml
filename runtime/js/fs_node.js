...
class MlNodeFd extends MlFile {
  ...
  write(buf, buf_offset, len, raise_unix) {
    try {
      ...
      if (this.flags.noSeek || !this.seeked) {
        written = this.fs.writeSync(this.fd, buf, buf_offset, len);
      } else {
        written = this.fs.writeSync(this.fd, buf, buf_offset, len, this.offset);
      }
      this.offset += written;
      return written;
    } catch (err) {
  ...
  read(a, buf_offset, len, raise_unix) {
    try {
      var read;
      if (this.flags.noSeek || !this.seeked) {
        read = this.fs.readSync(this.fd, a, buf_offset, len);
      } else {
