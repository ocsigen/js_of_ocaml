...
(@then
   ...
   (import "wasi_memory" "blit_memory_to_string"
      (func $blit_memory_to_string (param i32 i32) (result (ref $string))))
...
(@then
   ...
   (func $normalize_prefix (param $prefix (ref $string)) (result (ref $string))
      (local $i i32) (local $len i32) (local $c i32) (local $res (ref $string))
      (local.set $len (array.len (local.get $prefix)))
      (loop $loop
         (if (i32.lt_u (local.get $i) (local.get $len))
            (then
               (local.set $c
                  (array.get $string (local.get $prefix) (local.get $i)))
               ...
               (if (i32.eq (local.get $c) (i32.const 46)) ;; '.'
                  (then
                     (if (i32.eq (local.get $i)
                        ...
                        (else
                           (local.set $c
                              (array.get $string (local.get $prefix)
   ...
   (global $current_dir (mut (ref $string)) (@string ""))

   (@string $root_dir "/")

   (func $make_absolute
      (param $path (ref $string)) (result (ref $string))
      (local $need_slash i32) (local $i i32) (local $abs_path (ref $string))
      (if (i32.eqz (array.len (local.get $path)))
         (then ;; empty path
            (return (global.get $current_dir))))
      (if (i32.eq (i32.const 47) ;; '/'
             (array.get_u $string (local.get $path) (i32.const 0)))
         (then ;; absolute path
         ...
         (then
            (if (i32.and
                   (i32.eq (i32.const 46) ;; '.'
                      (array.get_u $string (local.get $path) (i32.const 0)))
                   (i32.eq (i32.const 47) ;; '/'
                      (array.get_u $string (local.get $path) (i32.const 1))))
               (then ;; starts with "./"
      ...
      (local.set $need_slash
         (if (result i32) (array.len (global.get $current_dir))
            (then
               (i32.ne (i32.const 47) ;; '/'
                  (array.get_u $string (global.get $current_dir)
      ...
      (array.set $string (local.get $abs_path)
         (array.len (global.get $current_dir))
         (i32.const 47)) ;; '/'
      (array.copy $string $string
         (local.get $abs_path)
   ...
   (func $wasi_chdir (export "wasi_chdir") (param $name (ref eq))
      (local $abs_path (ref $string)) (local $path (ref $string)) (local $i i32)
      (local.set $abs_path
         (call $make_absolute (ref.cast (ref $string) (local.get $name))))
      ...
      (loop $loop
         (if (i32.ge_s (local.get $i) (i32.const 0))
            (then
               (if (i32.eq (i32.const 47) ;; '/'
                      (array.get $string (local.get $abs_path) (local.get $i)))
                  (then
                     (local.set $i (i32.sub (local.get $i) (i32.const 1)))
                     (br $loop))))))
      (local.set $i (i32.add (local.get $i) (i32.const 1)))
      (if (i32.lt_u (local.get $i) (array.len (local.get $abs_path)))
         (then
            (local.set $path (array.new $string (i32.const 0) (local.get $i)))
            (array.copy $string $string
   ...
   (func $prefix_match
      (param $prefix (ref $string)) (param $path (ref $string)) (result i32)
      ...
      (if (i32.gt_u (array.len (local.get $path)) (local.get $len))
         (then
            (if (i32.ne (array.get_u $string (local.get $path) (local.get $i))
                   (i32.const 47))
               (then (return (i32.const 0))))))
      (loop $loop
         (if (i32.lt_u (local.get $i) (local.get $len))
   ...
   (func $resolve_abs_path
      ...
      (loop $loop
         (if (i32.lt_u (local.get $i) (array.len (local.get $path)))
            (then
               (if (i32.eq (array.get_u $string (local.get $path) (local.get $i))
                      (i32.const 47)) ;; 47
                  (then
                     (local.set $i (i32.add (local.get $i) (i32.const 1)))
                     (br $loop)))
   ...
   (func (export "wasi_resolve_path")
      ...
      (local $res (tuple i32 (ref $string)))
      (local $p i32)
      (local.set $res
         (call $resolve_abs_path
            (call $make_absolute
               (ref.cast (ref $string) (local.get $vpath)))))
...
(@then
   (func (export "caml_sys_read_directory")
      ...
      (local $i i32) (local $s (ref $string))
      (local.set $p (call $caml_sys_resolve_path (local.get $name)))
      (local.set $buffer (call $get_buffer))
      (local.set $res
         (call $path_open
            ...
            (if (i32.eq (local.get $namelen) (i32.const 2))
               (then
                  (br_if $loop
                     (i32.and
                        (i32.eq (i32.const 46)
                           (array.get_u $string (local.get $s) (i32.const 0)))
                        (i32.eq (i32.const 46)
                           (array.get_u $string (local.get $s) (i32.const 1))))))
               (else
                  (if (i32.eq (local.get $namelen) (i32.const 2))
                     (then
                        (br_if $loop
