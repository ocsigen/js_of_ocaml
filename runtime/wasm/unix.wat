...
(@then
   ...
   (import "wasi_memory" "blit_memory_to_substring"
      (func $blit_memory_to_substring (param i32 (ref $string) i32 i32)))
   (import "wasi_memory" "blit_substring_to_memory"
      (func $blit_substring_to_memory (param i32 (ref $string) i32 i32)))
...
(@else
   ...
   (func (export "unix_rewinddir") (export "caml_unix_rewinddir")
      (param (ref eq)) (result (ref eq))
      (call $caml_invalid_argument
         (@string "rewinddir not implemented"))
      (ref.i31 (i32.const 0)))
))
         ...
         (then
            (throw $ocaml_exception
               (array.new_fixed $block 5
                  ...
                  (array.new_data $string $link (i32.const 0) (i32.const 4))
                  (global.get $no_arg)))))
      (try
         (do
            (call $link
...
(@then
   ...
   (func (export "unix_symlink") (export "caml_unix_symlink")
      ...
      (result (ref eq))
      (local $path (ref $string))
      (local $len i32)
      (local $op i32)
      (local $np (tuple i32 i32 i32))
      (local $flags i32)
      (local $res i32)
      (local.set $path (ref.cast (ref $string) (local.get $o)))
...
(@then
   (global $access_flags (ref $flags)
      (array.new_fixed $flags 4
         ...
         (i32.const 1) ;; No notion of excecutable, so use readable instead
   ))
   ...
   (func (export "unix_access") (export "caml_unix_access")
      ...
      (local $flags i32) (local $file_type i32)
      (local $rights_inheriting i64) (local $expected i64)
      (local.set $flags
      ...
      (local.set $file_type (i32.load8_u offset=16 (local.get $buffer)))
      (local.set $res
         (call $fd_fdstat_get
            (tuple.extract 3 0 (local.get $p))
            (local.get $buffer)))
      (local.set $rights_inheriting (i64.load offset=16 (local.get $buffer)))
      (if (i32.and (local.get $flags) (i32.const 1))
         (then
            (local.set $expected
               (select
                  (i64.const 0x4000) ;; readdir
                  (i64.const 0x2) ;; read
                  (i32.eq (local.get $file_type) (i32.const 3)))))) ;; directory?
      (if (i32.and (local.get $flags) (i32.const 1))
         (then
            (local.set $expected
               (i64.or (local.get $expected)
                  (i64.const 0x40))))) ;; write
      (if (i64.ne (local.get $expected)
             (i64.and (local.get $rights_inheriting) (local.get $expected)))
         (then
            (call $caml_unix_error (i32.const 2) ;; EACCES
               (@string "access") (local.get $path))))
      (ref.i31 (i32.const 0)))
)
(@else
   (global $access_flags (ref $flags)
...
(@then
   (func (export "unix_write") (export "caml_unix_write")
      ...
      (local $fd i32) (local $s (ref $string))
   ...
   (func (export "unix_read") (export "caml_unix_read")
      ...
      (call $blit_memory_to_substring (local.get $buffer)
         (ref.cast (ref $string) (local.get $vbuf))
   ...
   (func (export "unix_write_bigarray") (export "caml_unix_write_bigarray")
      ...
      (local $fd i32) (local $data (ref $data)) (local $buf (ref $string))
