...
(@then
   ...
   (func (export "caml_sys_getenv") (export "caml_sys_unsafe_getenv")
      ...
      (local $var (ref $string)) (local $i i32) (local $j i32)
      (local $len i32) (local $s i32) (local $c i32)
      (call $initialize_env)
      (local.set $var (ref.cast (ref $string) (local.get $name)))
         ...
         (loop $loop
            (if (i32.lt_u (local.get $i) (local.get $len))
               (then
                  (br_if $not_found
                     (i32.eq (i32.const 61) ;; '='
                         (array.get_u $string (local.get $var) (local.get $i))))
...
(@else
   ...
   (data $command_not_implemented "Sys.command not implemented")

(@if wasi
(@then
   (func (export "caml_sys_system_command")
      (param (ref eq)) (result (ref eq))
      (call $caml_invalid_argument (array.new_data $string $command_not_implemented (i32.const 0) (i32.const 27)))
...
(@if wasi
(@then
   (func $on_windows (result i32) (i32.const 0))
))
         ...
         (if (result (ref eq)) (i32.gt_u (local.get $errno)
            ...
            (else
               (array.get $block (global.get $error_messages)
                  (local.get $errno)))))
      (if (ref.test (ref $string) (local.get $arg))
         (then
            (local.set $msg
