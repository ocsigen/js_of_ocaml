...
function caml_bigstring_memcmp(s1, pos1, s2, pos2, len) {
  for (let i = 0; i < len; i++) {
    const a = caml_ba_get_1(s1, pos1 + i);
    const b = caml_ba_get_1(s2, pos2 + i);
    if (a < b) return -1;
    if (a > b) return 1;
  }
  return 0;
...
function caml_bigstring_blit_ba_to_ba(ba1, pos1, ba2, pos2, len) {
  ...
  const slice = ba1.data.subarray(ofs1, ofs1 + len);
...
function caml_bigstring_blit_string_to_ba(str1, pos1, ba2, pos2, len) {
  ...
  const slice = caml_uint8_array_of_string(str1).slice(pos1, pos1 + len);
...
function caml_bigstring_blit_bytes_to_ba(str1, pos1, ba2, pos2, len) {
  ...
  const slice = caml_uint8_array_of_bytes(str1).slice(pos1, pos1 + len);
  ba2.data.set(slice, ofs2);
  return 0;
}
...
function caml_bigstring_blit_ba_to_bytes(ba1, pos1, bytes2, pos2, len) {
  ...
  const slice = ba1.data.slice(ofs1, ofs1 + len);
