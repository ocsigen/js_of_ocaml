...
let setup_toplevel () =
  ...
  then
    let header3 =
      Printf.sprintf
        "     'JsooTop.get_camlp4_syntaxes ()' to get loaded syntax extensions"
    in
      ...
      e##.onclick :=
        Dom_html.handler (fun _ ->
            ...
            let append_url str =
              let dom =
                Tyxml_js.Html.(
                  p [pcdata "Share this url : "; a ~a:[a_href str] [pcdata str]])
              in
            ...
            Lwt.async (fun () ->
                Lwt.catch
                  ...
                  (fun exn ->
                    Format.eprintf "Could not generate short url. reason: %s@."
