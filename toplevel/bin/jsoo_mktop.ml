...
module Camlp4 : CAMLP4 = struct
  (* We do the same as Camlp4Bin call Camlp4.Register.iter_and_take_callbacks
     after Every load of syntax module to initialize them. Camlp4 do it after
     dynlink; Since we statically link modules, we have to link dummy modules
...
let usage () =
  ...
  Format.eprintf
    " -top-syntax [pkg]\t\tInclude syntax extension provided by [pkg] findlib \
     package@." ;
  Format.eprintf
...
let _ =
  try
    ...
    in
    execute
      (["jsoo_mkcmis"; "-o"; "exported-unit.cmis.js"] @ !export @ toplevel_unit) ;
