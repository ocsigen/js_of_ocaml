  ...
  let cps_call_or_direct ~arity f args =
    ...
    block_expr
      ...
      (let* () =
       ...
       in
       (* The function has no CPS version. *)
       let* res =
         if n = 1
         then
           let* ty, funct = load_function_pointer ~cps:false ~arity:1 (load f) in
           let* args = expression_list Fun.id direct_args in
           let* closure = load f in
           return (W.Call_ref (ty, funct, args @ [ closure ]))
         else
           let* apply = need_apply_fun ~cps:false ~arity:n in
           let* args = expression_list Fun.id direct_args in
           let* closure = load f in
...
module Closure = struct
  ...
  (* [cps_version] is the CPS version of [f], with double translation (see
     ...
     versions, and is shared by them. *)
  let translate ~context ~closures ~cps ?cps_version ~no_code_pointer f =
    ...
    (* Both versions may be called through the closure *)
    let no_code_pointer = no_code_pointer && not pair in
