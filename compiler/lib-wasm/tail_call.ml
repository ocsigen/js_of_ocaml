...
let rewrite_tail_call ~return_exn ~no_tail_call ~y i =
  ...
  | LocalSet (x, Br_on_null (_, Call (symb, l)))
    when return_exn && Code.Var.equal x y && not (Code.Var.Hashtbl.mem no_tail_call symb)
    -> Some (Wasm_ast.Return_call (symb, l))
  | _ -> None
...
let rec instruction ~return_exn ~no_tail_call ~tail i =
  ...
  | Push (Call_ref _) -> i
  | Return (Some (Br_on_null (_, Call (symb, l))))
    when return_exn && not (Code.Var.Hashtbl.mem no_tail_call symb) ->
      Return_call (symb, l)
  | Push (Br_on_null (_, Call (symb, l)))
    when return_exn && tail && not (Code.Var.Hashtbl.mem no_tail_call symb) ->
      Return_call (symb, l)
