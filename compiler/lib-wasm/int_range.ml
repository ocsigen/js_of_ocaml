...
let max_i31s = lazy 0x3fffffffL

let min_i31s = lazy (-0x40000000L)
...
let arg_range st env a =
  match a with
  | Pc (Int c) ->
      let c = Int64.of_int32 (Targetint.to_int32 c) in
      Range (c, c)
  | Pc _ -> Top
...
let eval_expr st env x e =
  ...
  | Prim ((Not | IsInt | Eq | Neq | Lt | Le | Ult), _) -> Range (0L, 1L)
...
let counter_step st x y =
  ...
  | Expr_def
    ...
    when Var.equal x x' -> (
      match Int64.of_int32 (Targetint.to_int32 c) with
      | 1L -> Some `Up
      | -1L -> Some `Down
      | _ -> None)
  | Expr_def (Prim (Extern ("%int_sub", _), [ Pv x'; Pc (Int c) ]), _)
    when Var.equal x x' && Int64.(Int64.of_int32 (Targetint.to_int32 c) = 1L) ->
      Some `Down
...
let bound_of_arg a =
  match a with
  | Pv z -> Some (B_var z)
  | Pc (Int c) -> Some (B_const (Int64.of_int32 (Targetint.to_int32 c)))
  | Pc _ -> None
...
let f ~global_flow_state ~global_flow_info p =
  let t = Timer.make () in
  let max_t = Lazy.force max_i31s in
  ...
  let widened x =
    match st.defs.(Var.idx x) with
    | Expr_def
        ( (Field _ | Apply _ | Prim ((Array_get | Extern ("caml_array_unsafe_get", _)), _))
        , _ ) -> true
    | Expr_def _ -> false
