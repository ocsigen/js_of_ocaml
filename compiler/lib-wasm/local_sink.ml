...
let rec collect_rw rw (e : W.expression) =
  ...
  (* [Pop] reads the implicit Wasm value stack, not a local, so it
     contributes nothing to [reads]/[writes]. This means the
     stack-ordering dependency between a [Push] and a later [Pop] is not
     modelled here: sinking a [Pop]-bearing [e] across a [Push] would let
     the [Pop] consume the just-pushed value instead of the original
     stack top. This is safe only because codegen consumes every [Pop]
     immediately (e.g. try/catch results are [Push]ed and popped right
     away), so a [local.set]/[local.get] pair never straddles a [Push] of
     an unrelated value. *)
