...
module Domain = struct
  ...
  let rec print f t =
    ...
    | Number Int -> Format.fprintf f "int"
...
let prim_type prim =
  ...
  | "caml_lessequal"
  | "caml_equal"
  | "caml_compare" -> Number Int
  | "caml_int32_bswap" -> Number Int32
  | "caml_nativeint_bswap" -> Number Nativeint
  | "caml_int64_bswap" -> Number Int64
  | "caml_int32_compare" -> Number Int
  | "caml_nativeint_compare" -> Number Int
  | "caml_int64_compare" -> Number Int
  | "caml_string_get32" -> Number Int32
  | "caml_string_get64" -> Number Int64
  | "caml_bytes_get32" -> Number Int32
  | "caml_bytes_get64" -> Number Int64
  | "caml_bytes_set32" -> Number Int
  | "caml_bytes_set64" -> Number Int
  | "caml_lxm_next" -> Number Int64
  | "caml_ba_uint8_get32" -> Number Int32
  | "caml_ba_uint8_get64" -> Number Int64
  | "caml_ba_uint8_set32" -> Number Int
  | "caml_ba_uint8_set64" -> Number Int
  | "caml_nextafter_float" -> Number Float
  | "caml_classify_float" -> Number Int
  | "caml_ldexp_float" -> Number Float
  | "caml_erf_float" -> Number Float
  | "caml_erfc_float" -> Number Float
  | "caml_float_compare" -> Number Int
  | "caml_floatarray_unsafe_get" -> Number Float
  | "caml_bytes_unsafe_get" -> Number Int
  | "caml_string_unsafe_get" -> Number Int
  | "caml_bytes_get" -> Number Int
  | "caml_string_get" -> Number Int
  | "caml_ml_string_length" -> Number Int
  | "caml_ml_bytes_length" -> Number Int
  | "%direct_obj_tag" -> Number Int
  | "caml_add_float"
  | "caml_sub_float"
  | "caml_mul_float"
  | "caml_div_float"
  | "caml_copysign_float" -> Number Float
  | "caml_signbit_float" -> Number Float
  | "caml_neg_float"
  | "caml_abs_float"
  | "caml_ceil_float"
  | "caml_floor_float"
  ...
  | "caml_int32_or"
  | "caml_int32_xor"
  | "caml_int32_div" -> Number Int32
  | "caml_int32_mod"
  | "caml_int32_shift_left"
  | "caml_int32_shift_right"
  | "caml_int32_shift_right_unsigned" -> Number Int32
  | "caml_int32_to_int" -> Number Int
  ...
  | "caml_nativeint_to_int" -> Number Int
  | "caml_nativeint_of_int" -> Number Nativeint
  | "caml_int_compare" -> Number Int
  | _ -> Top

let propagate st approx x : Domain.t =
  match st.state.defs.(Var.idx x) with
  | Phi { known; others } -> Domain.join_set ~others (fun y -> Var.Tbl.get approx y) known
  | Expr e -> (
      match e with
      | Constant c -> constant_type c
      | Closure _ -> Top
      | Block (_, lst, _, _) ->
          let m = Var.ISet.mem st.state.possibly_mutable x in
          Tuple (Array.map ~f:(fun y -> if m then Top else Var.Tbl.get approx y) lst)
      | Field (y, n, _) -> (
              ...
              Domain.join_set
                ...
                (fun z ->
                  ...
                  | Expr (Block (_, lst, _, _)) ->
                      let m = Var.ISet.mem st.state.possibly_mutable z in
                      if m
                      then Top
                      else
                        Array.fold_left
      ...
      | Prim ((Vectlength | Not | IsInt | Eq | Neq | Lt | Le | Ult), _) -> Number Int
