...
let f ~param_names ~param_types ~locals instrs =
  ...
  then locals, instrs
    ...
    (* Dead-tee detection using the liveness already computed. A
         ...
         [live_out] here is the union over the node's NORMAL-flow
         successors only; it does not include the exception-edge
         liveness that [compute_live_ranges] adds via the [try_blocks]
         back-extension. That is sound because the input is in (loop-)
         SSA form: a local read by a catch handler is defined by a block
         that dominates the handler, and since the handler is reachable
         from anywhere in the protected body, no interior body point
         dominates it. Hence the defining [LocalTee] always sits BEFORE
         the [Try], never inside the body, and an interior tee can never
         be the sole definition of a variable observed only on the
         exceptional path. If that invariant were ever broken, an
         interior tee whose value is read only by a handler would be
