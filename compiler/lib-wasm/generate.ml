...
module Generate (Target : Target_sig.S) = struct
  ...
  let () =
    ...
    register_arith_bin_prim
      ...
      `Pure
      ~typ:float32_u
      (fun f g -> float32_bin_op Min f g);
    register_arith_bin_prim
      "caml_simd_float32_max_bytecode"
      `Pure
      ~typ:float32_u
      (fun f g -> float32_bin_op Max f g);
    register_un_prim
  ...
  let function_return_type (p : Code.program) name_opt ~toplevel_name pc =
    let* ty =
      Code.preorder_traverse
        ...
        (fun pc ty ->
          ...
          | Return x ->
              ...
              value_type_lub
                ty
                (match Option.value ~default:Type.value ty' with
                | I32 -> Ref { nullable = false; typ = I31 }
                | ty' -> ty')
      ...
      function_body
        ...
        ~body:
          (let* () =
           ...
           | Some ty -> return ty
           | None -> function_return_type p name_opt ~toplevel_name pc)
    in
