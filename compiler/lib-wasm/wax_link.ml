...
let value_to_define (v : value) : Wax.Define.value =
  match v with
  | Bool b -> Wax.Define.Bool b
  | String s -> Wax.Define.String s
  | Version (a, b, c) -> Wax.Define.Version (a, b, c)
...
let with_preprocessed_files ~variables ~inputs action =
  ...
  let defines =
    Wax.Define.of_list
      ...
      :: ("oxcaml", Wax.Define.Bool Config.oxcaml)
      :: ("introspect", Wax.Define.Bool Config.introspect)
      :: List.filter_map variables ~f:(fun (k, v) ->
          if String.equal k "name-wasm-functions"
          then None
          else Some (k, value_to_define v)))
  in
  (* [warn_unused] off: the same sources are assembled under several sets of
     [defines], and a declaration whose only uses are conditional is
  ...
  let assemble ~filename contents =
    ...
    then
      ...
      Wax.wax_to_binary ~defines ~validate:true ~warn_unused:false ~filename contents
    else
      Wax.wat_to_binary
        ~defines
        ~name_functions
