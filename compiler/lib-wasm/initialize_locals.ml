...
let rec rewrite_expression uninitialized (e : Wasm_ast.expression) =
  ...
  | LocalGet i ->
      if Hashtbl.mem uninitialized i then RefCast (Hashtbl.find uninitialized i, e) else e
  | LocalTee (i, e') ->
  ...
  List.iter
    ~f:(fun (var, typ) ->
      ...
      | Ref ({ nullable = false; typ } as ref_typ) ->
          if not (has_default typ) then Hashtbl.add local_types var ref_typ)
    locals;
  scan_instructions ctx instrs;
  Hashtbl.filter_map_inplace
    (fun x ty -> if Code.Var.Set.mem x !(ctx.uninitialized) then Some ty else None)
    local_types;
  let locals =
    List.map
      ~f:(fun ((var, typ) as local) ->
        match typ with
        | Ref { nullable = false; typ } when Hashtbl.mem local_types var ->
            var, Wasm_ast.Ref { nullable = true; typ }
        | I32 | I64 | F32 | F64 | Ref _ -> local)
      locals
  in
  ( locals
  , List.filter_map
      ~f:(fun i ->
        if Hashtbl.mem local_types i
        then None
        else Some (Wasm_ast.LocalSet (i, RefI31 (Const (I32 0l)))))
      (Code.Var.Set.elements !(ctx.uninitialized))
    @ rewrite_instructions local_types instrs )
