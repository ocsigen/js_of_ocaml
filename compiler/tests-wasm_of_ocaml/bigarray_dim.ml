(* Bigarray dimensions are read inline. This checks the dimensions, and
   accesses whose indices come from memory, including out-of-bounds indices
   that do not fit in 32 bits with 63-bit integers. *)
