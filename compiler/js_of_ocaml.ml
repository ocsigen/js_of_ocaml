...
let temp_file_name =
  (* Inlined unavailable Filename.temp_file_name. Filename.temp_file gives us
     incorrect permissions. https://github.com/ocsigen/js_of_ocaml/issues/182 *)
...
let _ =
  ...
  | Util.MagicNumber.Bad_magic_version h ->
      ...
      in
      Format.eprintf
        "%s: Error: Your ocaml bytecode and the js_of_ocaml compiler have to \
         be compiled with the same version of ocaml.@."
        Sys.argv.(0) ;
      Format.eprintf
      ...
      Format.eprintf
        "%s: Error: Its seems that your ocaml bytecode has been compiled with \
         %s version of ocaml.@."
        Sys.argv.(0) comp ;
      exit 1
  | Failure s ->
