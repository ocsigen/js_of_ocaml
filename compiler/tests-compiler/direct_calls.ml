...
|}
  ...
  [%expect
    ...
    function test3(x, cont){
     ...
     var M1 = F(), M2 = F(), _t_ = M2[2].call(null, 2);
     return cont([0, M1[2].call(null, 1), _t_]);
    }
    //end
    function test4(x, cont){
     function F(symbol){
      ...
      function f(x, cont){
       return caml_trampoline_cps_call3(Stdlib_Printf[2], _q_, x, cont);
      }
      return [0, , f];
     }
     var M1 = F(), M2 = F();
