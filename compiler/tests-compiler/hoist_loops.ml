...
let%expect_test "simple for-loop is extracted into a helper" =
  ...
  [%expect
    ...
    (function(globalThis){
       ...
       var Stdlib = runtime.caml_get_global("Stdlib"), dummy = 0, r = [0, 0];
       (function(_a_){
          var i = _a_;
          for(;;){
           r[1] = r[1] + i | 0;
...
let%expect_test "two independent loops, each with its own helper" =
  ...
  [%expect
    ...
    (function(globalThis){
       ...
       var dummy = 0, Stdlib = runtime.caml_get_global("Stdlib"), s = [0, 0];
       (function(_a_){
          var i = _a_;
          for(;;){
           s[1] = s[1] + i | 0;
       ...
       var
        ...
        n =
          function(loop_init, _a_){
             var n = loop_init, i = _a_;
             for(;;){
              var n$0 = n + i | 0, _a_ = i + 1 | 0;
              if(5 === i) return [0, n$0];
              n = n$0;
              i = _a_;
             }
       ...
       (function(_a_){
          ...
          for(;;){
           ...
           for(;;){
            ...
            _a_ = j + 1 | 0;
            if(3 === j){_a_ = i + 1 | 0; if(3 === i) return; i = _a_; break;}
            j = _a_;
           }
          }
         }
...
(* The loop is directly inside a [try] body: the [Poptrap] leaving the
   loop must be performed by the caller, after the call to the helper, so
   that exceptions raised after the loop are not caught by the handler. *)
let loop_in_try =
  {|
  let r = ref 0
  let f = Sys.opaque_identity (fun i -> if i = 100 then raise Exit)
  let g = Sys.opaque_identity (fun () -> if !r > 5 then raise Exit)
  let () =
    (try
      (try
        while !r < Sys.opaque_identity 10 do
          f !r;
          incr r
        done
      with Exit -> print_endline "wrong handler");
      g ()
    with Exit -> print_endline "right handler");
    let x =
      try
        for i = 0 to Sys.opaque_identity 10 do
          (try if i = 3 then raise Not_found with Not_found -> r := !r + 100);
          f i;
          r := !r + i
        done;
        !r
      with Exit -> 0
    in
    (try g () with Exit -> print_endline "right handler");
    print_int x;
    print_newline ()
  |}

let%expect_test "loop inside a try body" =
  let prog = Util.compile_and_parse loop_in_try in
  Util.print_program prog;
  [%expect
    {|
           (function(globalThis){
              ...
              function f(i){
               var _b_ = 100 === i ? 1 : 0;
               if(_b_) throw caml_maybe_attach_backtrace(Stdlib[3], 1);
               return _b_;
              }
              var r = [0, 0];
              function g(_b_){
              ...
              var
               _b_ = 0,
               _b_ = caml_string_of_jsbytes("wrong handler"),
               _a_ = caml_string_of_jsbytes("right handler"),
               cst_right_handler = caml_string_of_jsbytes("right handler");
              try{
               try{(function(){for(;;){if(r[1] >= 10) return; f(r[1]); r[1]++;}}());}
               catch(exn){
                var exn$2 = caml_wrap_exception(exn);
                if(exn$2 !== Stdlib[3]) throw caml_maybe_attach_backtrace(exn$2, 0);
              ...
              try{
               _a_ =
                function(_b_){
                   var i = _b_;
                   for(;;){
                    try{if(3 === i) throw caml_maybe_attach_backtrace(Stdlib[8], 1);}
                    catch(exn$0){
                     var exn = caml_wrap_exception(exn$0);
               ...
               if(exn$0 !== Stdlib[3]) throw caml_maybe_attach_backtrace(exn$0, 0);
               x = 0;
              }
              try{g(0);}
              catch(exn){
               var exn$1 = caml_wrap_exception(exn);
               if(exn$1 !== Stdlib[3]) throw caml_maybe_attach_backtrace(exn$1, 0);
               caml_call1(Stdlib[46], cst_right_handler);
              }
              caml_call1(Stdlib[44], x);
              caml_call1(Stdlib[47], 0);
              runtime.caml_register_global([0, r, f, g], "Test");
              return;
             }
             (globalThis));
           //end
...
let%expect_test "loop inside a try body: execution" =
  Stdlib.List.iter
    (fun effects -> Util.compile_and_run ~effects loop_in_try)
    [ `Disabled; `Cps; `Double_translation ];
  [%expect
    {|
           right handler
           right handler
