...
let%expect_test "test-compiler/lib-effects/test1.ml" =
    ...
    function exceptions$0(s){
     ...
     try{
      if(caml_string_equal(s, cst))
       throw caml_maybe_attach_backtrace(Stdlib[8], 1);
      var _z_ = [0, [0, caml_doublecall1(Stdlib[79], cst_toto), n, m]];
      return _z_;
     }
     catch(_C_){
  ...
  [%expect
    ...
    function handler_is_loop$0(f, g, l){
     try{var _l_ = caml_doublecall1(f, 0); return _l_;}
     catch(_m_){
      var l$0 = l;
      for(;;){
  ...
  [%expect
    ...
    function handler_is_merge_node$0(g){
     try{var _g_ = caml_doublecall1(g, 0), s = _g_;}catch(_h_){var s = cst$1;}
     return caml_doublecall2(Stdlib[28], s, cst_aaa);
    }
    //end
    function handler_is_merge_node$1(g, cont){
     function _d_(s){return caml_cps_call3(Stdlib[28], s, cst_aaa, cont);}
     caml_push_trap(function(_f_){return _d_(cst$1);});
     return caml_cps_call2(g, 0, function(_e_){caml_pop_trap(); return _d_(_e_);});
    }
    //end
    var
