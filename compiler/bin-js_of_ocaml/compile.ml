...
let run
  ...
  Config.Flag.(match target_env with `Nodejs | `Isomorphic -> enable "include-node"  | _ -> disable "include-node");
  Config.Flag.(match target_env with `Browser | `Isomorphic -> enable "include-browser"  | _ -> disable "include-browser");
  let t = Timer.make () in
  let include_dir =
