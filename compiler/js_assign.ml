...
module Preserve : Strategy = struct
  ...
  *)
  ...
  type t = {
    size   : int;
    mutable scopes : Js_traverse.t list
  }
  let create size = { size ; scopes = [] }

  let record_block t scope ~catch:_ _param =
  ...
  let allocate_variables t ~count:_ =
    ...
    List.iter (fun state ->
      ...
      let no_name_idx = ref 0 in
      let _assigned = S.fold (fun var assigned ->
        if names.(Var.idx var) <> ""
        then assigned
        else
          let name =
            ...
            | None ->
              while StringSet.mem (Printf.sprintf "$$%d" !no_name_idx) assigned do
                incr no_name_idx
              done;
              Printf.sprintf "$$%d" !no_name_idx
          in
          names.(Var.idx var) <- name;
          StringSet.add name assigned
      ) state.Js_traverse.def assigned in
      ()
    ) t.scopes;
    names
...
let program' (module Strategy : Strategy) p =
  ...
  let names = Strategy.allocate_variables state mapper#state.Js_traverse.count in
