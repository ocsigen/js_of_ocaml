...
let debuginfo_of_var v =
  match Code.Var.get_addr v with
  | None -> J.N
  | Some pc -> J.Loc pc
          ...
          List.fold_left
            (fun (acc,instrs) js ->
               ...
               | J.EArr _ ->
                   ...
                   Var.name v "partial";
                   let instrs =
                     J.Variable_statement ([J.V v, Some (js,J.N)],J.N) :: instrs in
...
let access_queue_may_flush queue v x =
  ...
  let _,instrs,queue = List.fold_left (fun (deps,instrs,queue) ((y,elt) as eq) ->
      ...
      then (Code.VarSet.add ( y) deps),
        ((J.Variable_statement ([ J.V y, Some (elt.ce,debuginfo_of_var y)],J.N)) ::instrs),
        queue
...
let enqueue expr_queue prop x ce cardinal acc =
  ...
  in
  (instrs @ acc , (x, {prop; ce; cardinal; deps}) :: expr_queue)
...
let parallel_renaming ctx params args continuation queue =
  ...
  List.fold_left
    ...
    fun queue ->
      ...
      let (st, queue) = flush_queue queue px (instrs@[J.Variable_statement ([ J.V y, Some (cx,debuginfo_of_var y)],J.N)])
      in
...
and translate_closures ctx expr_queue l =
  ...
  | [(x, vars, cl)] :: rem ->
      ...
      let (st, expr_queue) =
        ...
        | _ -> flush_queue expr_queue flush_p
                 [J.Variable_statement ([J.V x, Some (cl,debuginfo_of_var x)],J.N)]
      in
  ...
  | l :: rem ->
      ...
      in
      let defs =
        List.map (fun (x, _, cl) -> (J.V x, Some (cl,debuginfo_of_var x))) l in
      let statement =
        if vars = [] then
          ...
          let assgn =
            List.fold_left
              (fun (l, n) (x, _, _) ->
                 ((J.V x,
                   Some (J.EAccess (J.EVar (J.V tbl), int n), debuginfo_of_var x)) :: l,
                  n + 1))
              ([], 0) l
          in
          J.Variable_statement
          ...
          Let (x, e) ->
            ...
            *)
            ...
            | _ -> flush_queue expr_queue prop
                     (instrs@[J.Variable_statement ([J.V x, Some (ce,debuginfo_of_var x)],J.N)])
            end
        | Set_field (x, n, y) ->
          ...
          if AddrSet.cardinal new_frontier > 1 then begin
            ...
            let switch =
              if Array.length cases > 2 then
                Code.Switch (x, cases, [||], Code.DebugAddr.no)
              else
                Code.Cond (IsTrue, x, cases.(1), cases.(0), Code.DebugAddr.no)
            in
            st.blocks <-
              AddrMap.add idx
...
and compile_conditional st queue pc last handler backs frontier interm succs =
  ...
  match last with
    Return (x,pc) ->
      let ((px, cx), queue) = access_queue queue x in
      flush_all queue [J.Return_statement (Some cx,J.Loc pc)]
  | Raise (x,pc) ->
      let ((px, cx), queue) = access_queue queue x in
      flush_all queue [J.Throw_statement (cx,J.Loc pc)]
  | Stop pc ->
      flush_all queue [J.Return_statement (None,J.Loc pc)]
  | Branch (cont,pc) ->
  ...
  | Cond (c, x, cont1, cont2, pc) ->
      ...
      flush_all queue
        (compile_if st e ~pc:(J.Loc pc) cont1 cont2 handler backs frontier interm succs)
  ...
  if debug () then begin
    match last with
      Branch _ | Poptrap _ | Pushtrap _ | Return _ | Raise _ | Stop _ -> ()
...
(*FIX: we should flush only the variables we need rather than doing this;
       do the same for closure free variables *)
                 match 2 (*ctx.Ctx.live.(Var.idx y)*) with
                   0 -> assert false
                 | 1 -> enqueue queue px y cx 1 []
                 | _ -> flush_queue queue px
                          [J.Variable_statement ([J.V y, Some (cx,debuginfo_of_var y)],J.N)]
               in
...
and compile_branch_selection pc interm =
  try
    ...
    J.Variable_statement ([J.V x, Some (int i,debuginfo_of_var x)],J.Loc (DebugAddr.of_addr pc)) ::
