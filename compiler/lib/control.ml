...
let concat_blocks pc instr params handler args params' instr' last' =
  (* This is only valid because we know that the params only occur in the block *)
...
let rec block_simpl pc (preds, entries, blocks) =
  ...
  | Branch (pc', args) ->
      ...
      if false
         (*XXX FIX! not (AddrSet.mem pc' entries) && AddrSet.cardinal
           (AddrMap.find pc' preds) = 1 && block'.params = [] && block'.handler
           = block.handler *)
      then (
  ...
  | Cond (c, x, cont1, cont2) -> (
      if cont1 = cont2
      then
        let blocks = AddrMap.add pc {block with branch= Branch cont1} blocks in
        ...
        | Some cont1' ->
            ...
            let preds =
              AddrMap.add pc1'
                (AddrSet.add pc (AddrSet.remove pc1 (AddrMap.find pc1' preds)))
                preds
            in
            let blocks =
              AddrMap.add pc
