...
(* XXX Beware automatic semi-colon insertion... a=b ++c is not the same as a=b
   ++c ===> see so-called "restricted productions": the space cannot be
   replaced by a newline in the following expressions: e ++, e --, continue e,
   break e, return e, throw e *)
  ...
  and statement ?(last = false) f (s, loc) =
    ...
    | Expression_statement e ->
        ...
        then (
          ...
          PP.start_group f 0 ; expression 0 f e ; last_semi () ; PP.end_group f )
    ...
    | Throw_statement e ->
        ...
        PP.end_group f ;
        PP.end_group f
    (* There must be a space between the return and its argument. A line return
       would not work *)
...
let need_space a b =
  ...
  (* do not generate end_of_line_comment. handle the case of "num / /* comment
     */ b " *)
  || (a = '/' && b = '/')
