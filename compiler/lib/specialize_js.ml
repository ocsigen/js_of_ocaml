...
let specialize_instr info i rem =
  ...
  | Let
      ...
      , Prim
          ( Extern
              (("caml_js_var" | "caml_js_expr" | "caml_pure_js_expr") as prim)
          , [y] ) ) ->
      ( match the_string_of info y with
  ...
  | Let (x, Prim (Extern "caml_js_object", [a])) ->
      ( try
          ...
          Let
            ( x
            , Prim
                (Extern "%caml_js_opt_object", List.flatten (Array.to_list a))
            )
        with Exit -> i )
      :: rem
...
let rec specialize_instrs info checks l =
  ...
  | i :: r -> (
    (* We make bound checking explicit. Then, we can remove duplicated bound
       ...
       access. The bound checking function returns the array, which allows to
