...
let apply_fun_raw =
  ...
  fun ctx f params exact trampolined cps ->
...
let generate_apply_fun ctx { arity; exact; trampolined; in_cps } =
  ...
  J.EFun
    ...
    , J.fun_
        (f :: params)
        [ ( J.Return_statement
              (Some (apply_fun_raw ctx f' params' exact trampolined in_cps))
          , J.N )
        ]
        J.N )
...
let apply_fun ctx f params exact trampolined in_cps loc =
  (* We always go through an intermediate function when doing CPS
     ...
     engines. *)
  if Config.Flag.inline_callgen () || (exact && (not trampolined) && not in_cps)
  then apply_fun_raw ctx f params exact trampolined in_cps
  else
    let y =
      Share.get_apply
...
let rec translate_expr ctx queue loc x e level : _ * J.statement_list =
  ...
  | Apply { f; args; exact } ->
      ...
      let e = apply_fun ctx f args exact trampolined in_cps loc in
      (e, prop, queue), []
