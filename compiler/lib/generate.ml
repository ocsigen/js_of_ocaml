...
(*XXX Patterns: => loops should avoid absorbing the whole continuation...
  ...
  preceding if statement when possible => if e1 then {if e2 then P else Q} else
  {if e3 then P else Q} => if e then return e1; return e2 => if e then var x =
  e1; else var x = e2; => while (true) {.... if (e) continue; break; }
...
(* like [List.map] except that it calls the function with an additional
   argument to indicate whether we're mapping over the last element of the list *)
...
module Share = struct
  ...
  let rec get_constant c t =
    ...
    | Tuple (_, args) -> Array.fold_left (fun t c -> get_constant c t) t args
    | _ -> t
        ...
        (fun _ block share ->
          List.fold_left
            (fun share i ->
              ...
              | Let
                  ( _
                  , Prim (Extern "%closure", [Pc (IString name | String name)])
                  ) ->
...
let flush_queue expr_queue prop (l : J.statement_list) =
  let instrs, expr_queue =
    if prop >= flush_p
    then expr_queue, []
    else List.partition (fun (_, elt) -> should_flush prop elt.prop) expr_queue
  in
  let instrs =
    List.map
      (fun (x, elt) ->
...
let rec dominance_frontier_rec st pc visited grey =
  ...
  then
    ...
    then
      ...
      List.fold_right
        (fun pc' (visited, grey) -> dominance_frontier_rec st pc' visited grey)
        s (visited, grey)
...
let rec resolve_node interm pc =
  try resolve_node interm (fst (AddrMap.find pc interm)) with Not_found -> pc
...
let generate_apply_fun ctx n =
  ...
  J.EFun
    ( None
    , f :: params
    , [ ( J.Statement (J.Return_statement (Some (apply_fun_raw ctx f' params')))
        , J.N ) ]
    , J.N )
...
(****)
(* when raising ocaml exception and [improved_stacktrace] is enabled, tag the
   ocaml exception with a Javascript error (that contain js stacktrace). {[
...
let rec translate_expr ctx queue loc _x e level : _ * J.statement_list =
  ...
  | Prim (p, l) ->
      let res =
        ...
        | IsInt, [x] ->
            (* JavaScript engines recognize the pattern 'typeof x==="number"';
               if the string is shared, less efficient code is generated. *)
            let (px, cx), queue = access_queue' ~ctx queue x in
            J.EBin (J.EqEqEq, J.EUn (J.Typeof, cx), str_js "number"), px, queue
        | Ult, [x; y] ->
            let (px, cx), queue = access_queue' ~ctx queue x in
            let (py, cy), queue = access_queue' ~ctx queue y in
...
and translate_instr ctx expr_queue loc instr =
  ...
  | Let (x, e) -> (
      ...
      (* We could inline more. size_v : length of the variable after
         serialization size_c : length of the constant after serialization num
         : number of occurrence size_c * n < size_v * n + size_v + 1 + size_c *)
  ...
  | Offset_ref (x, 1) ->
      ...
      let (_px, cx), expr_queue = access_queue expr_queue x in
      flush_queue expr_queue mutator_p
        [ ( J.Expression_statement (J.EUn (J.IncrA, J.EAccess (cx, J.ENum 1.)))
          , loc ) ]
  | Offset_ref (x, n) ->
      (* FIX: may overflow.. *)
      let (_px, cx), expr_queue = access_queue expr_queue x in
  ...
  then flush_all queue (compile_block st [] pc frontier interm)
    ...
    let body =
      ...
      | Code.Pushtrap ((pc1, args1), x, (pc2, args2), pc3s) ->
          ...
          let body =
            compile_branch st [] (pc1, args1) None AddrSet.empty inner_frontier
              new_interm
          in
...
and colapse_frontier st new_frontier interm =
  ...
  then (
    ...
    (* Put a limit: we are going to remove other branches to the members of the
       frontier (in compile_conditional), but they should remain in the
       frontier. *)
...
and compile_decision_tree st _queue handler backs frontier interm succs loc cx
    dtree =
...
and compile_conditional st queue pc last handler backs frontier interm succs =
  ...
  let res =
    ...
    | Switch (x, a1, [||]) ->
        ...
        flush_all queue code
    | Switch (x, a1, a2) ->
        (* The variable x is accessed several times, so we can directly refer
           to it *)
        (* We do not want to share the string "number". See comment for IsInt *)
        let b1 =
          compile_decision_tree st queue handler backs frontier interm succs
    ...
    | Some (x0, (h_pc, h_args)) ->
        ...
        let rec loop continuation old args params queue =
          ...
          | x :: args, y :: params ->
              ...
              in
              let x' = try Some (VarMap.find x m) with Not_found -> Some x in
              if Var.compare x x0 = 0 || x' = z
              then loop continuation old args params queue
              else
                let (px, cx), queue = access_queue queue x in
                let st, queue =
                  (*FIX: we should flush only the variables we need rather than
                    doing this; do the same for closure free variables *)
