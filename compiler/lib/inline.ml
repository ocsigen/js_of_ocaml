...
let rec small_function ~context info args =
  ...
  try
    List.iter2 args info.params ~f:(fun arg param ->
        if Var.Map.mem arg context.env && List.mem_assoc param ~map:relevant_params
        then
...
let inline ~aggressive ~inline_count p ~live_vars =
...
let f ~aggressive p live_vars =
  let previous_p = p in
  let inline_count = ref 0 in
  Code.invariant p;
  let t = Timer.make () in
  let p = inline ~aggressive ~inline_count p ~live_vars in
