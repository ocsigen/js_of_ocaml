      ...
      List.fold_left
        (fun closures i ->
          ...
          | Let (x, Closure (l, cont)) ->
              ...
              (* we can compute this once during the pass as the property won't
                 change with inlining *)
...
(* get new location put continuation at new location update closure body to
   return to this location make current block continuation jump to closure body *)
            ...
            | `Fail ->
                if live_vars.(Var.idx f) = 1
                   ...
                   (* inlining the code of an optimizable function could make
                      this code unoptimized. (wrt to Jit compilers) At the
                      moment, V8 doesn't optimize function containing
                ...
                then
                  ...
                  (* We do not really need this intermediate block. It just
                     avoid the need to find which function parameters are used
