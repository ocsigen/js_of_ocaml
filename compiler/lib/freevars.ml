...
let free_variables vars in_loop (pc, blocks, free_pc) =
    ...
    then (
      ...
      iter_block_free_vars
        (fun x ->
          ...
          then
            let fv =
              try AddrMap.find pc' !all_freevars with Not_found -> VarSet.empty
            in
  ...
  in
  traverse pc ;
  (* AddrMap.iter (fun pc fv -> if VarSet.cardinal fv > 0 then Format.eprintf
     ">> %d: %d@." pc (VarSet.cardinal fv)) !freevars; *)
  !freevars
