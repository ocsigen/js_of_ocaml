...
let eval_instr info i =
  ...
  | Let (x, Prim (Extern "caml_ml_string_length", [s])) -> (
      ...
      | Some c ->
          ...
          Let (x, c) )
  | Let
      (_, Prim (Extern ("caml_array_unsafe_get" | "caml_array_unsafe_set"), _))
    ->
      (* Fresh parameters can be introduced for these primitives in
         Specialize_js, which would make the call to [the_const_of] below fail. *)
      i
  | Let (x, Prim (IsInt, [y])) -> (
...
let eval_branch info = function
  ...
  | Switch (x, const, tags) as b -> (
    (* [the_case_of info (Pv x)] might be meaningless when we're inside a dead
       code. The proper fix would be to remove the deadcode entirely.
