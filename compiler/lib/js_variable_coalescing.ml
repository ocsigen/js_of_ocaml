  ...
  let rec visit_stmt ?(labels = []) context exit stmt =
    ...
    | Try_statement (body, catch, finally) ->
        ...
        (* Wrap [body_entry] in a fresh [Nop] shim whose only predecessor
           ...
           the body is a single [break]/[continue] that lands on the
           target of a [break]/[continue] from the catch, or when
           control-flow patterns generated by [Generate.compile_branch]
           cause the body's first statement to be a node already on the
           catch's forward chain. That would make the body end up inside
           catch's/finally's DFS subtree, violating
           [body_order < catch_order] in [compute_live_ranges].

           The shim also subsumes the previous "empty body"
           short-circuit: for an empty body, [body_entry] is now a
           distinct Nop node pointing at [finally_entry], not
           [finally_entry] itself, and nothing special is needed. *)
