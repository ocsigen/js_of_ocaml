...
module Debug : sig
  ...
  let find_ml_in_paths paths name =
    ...
    try Some (Util.find_in_path paths (uname ^ ".ml")) with Not_found -> (
      try Some (Util.find_in_path paths (name ^ ".ml")) with Not_found -> None )
...
module Blocks : sig
  ...
  let rec scan debug blocks code pc len =
    ...
    then
      ...
      | KNullaryCall ->
          let blocks =
            if Debug.mem debug (pc + 1) then AddrSet.add pc blocks else blocks
          in
          scan debug blocks code (pc + 1) len
      | KUnaryCall ->
          let blocks =
            if Debug.mem debug (pc + 2) then AddrSet.add pc blocks else blocks
          in
          scan debug blocks code (pc + 2) len
      | KBinaryCall ->
          let blocks =
            if Debug.mem debug (pc + 3) then AddrSet.add pc blocks else blocks
          in
...
module Constants : sig
  ...
  let rec parse x =
    ...
    then
      ...
      then Tuple (tag, Array.init (Obj.size x) (fun i -> parse (Obj.field x i)))
      else assert false
    else
...
module State = struct
  ...
  let rec list_start n l =
    ...
    then []
      ...
      match l with [] -> assert false | v :: r -> v :: list_start (n - 1) r
...
let rec compile_block blocks debug code pc state =
  ...
  then (
    ...
    if debug_parser ()
    then
      Format.eprintf "Compiling from %s to %d@." (string_of_addr pc) (limit - 1) ;
...
and compile infos pc state instrs =
  ...
  then
    ...
    | APPLY2 ->
        ...
        if debug_parser ()
        then
          Format.printf "%a = %a(%a, %a)@." Var.print x Var.print f Var.print y
            Var.print z ;
    ...
    | APPTERM2 ->
        ...
        if debug_parser ()
        then
          Format.printf "return %a(%a, %a)@." Var.print f Var.print x Var.print
            y ;
    ...
    | SETVECTITEM ->
        if debug_parser ()
        then
          Format.printf "%a[%a] = %a@." Var.print (State.accu state) Var.print
            (State.peek 0 state) Var.print (State.peek 1 state) ;
        let instrs =
    ...
    | C_CALL1 ->
        ...
        then (* This is a no-op *)
          ...
          if debug_parser ()
          then
            Format.printf "%a = ccall \"%s\" (%a)@." Var.print x prim Var.print
              y ;
          compile infos (pc + 2) state
...
let exe_from_channel ~includes ?(toplevel = false) ?(expunge = fun _ -> `Keep)
  ...
  then ()
    ...
    with Not_found -> (
      ...
      | `Full ->
          Util.warn
            "Warning: Program not linked with -g, original variable names and \
             locations not availalbe.@." ) ) ;
  let globals =
        ...
        List.fold_left
          (fun rem (name, const) ->
            ...
            :: Let
                 ( Var.fresh ()
                 , Prim
                     (Extern "caml_js_set", [Pv gdata; Pc (String name); Pv c])
                 )
            :: rem )
          body infos
      in
...
module Reloc = struct
  ...
  let step1 t compunit code =
    ...
    List.iter
      (fun name -> Hashtbl.add t.primitives name (Hashtbl.length t.primitives))
  ...
  let constants t =
    ...
    (* WARNING: [Obj.t array] is dangerous. Make sure we don't end up with an
       unboxed float array. *)
      ...
      (fun i var l ->
        ...
        | Some x when globals.is_const.(i) -> (
          ...
          | Some name ->
              Var.name x name ;
              Let
                (x, Prim (Extern "caml_js_get", [Pv gdata; Pc (IString name)]))
              :: l )
        | _ -> l )
      globals.vars []
  in
...
let from_channel ?(includes = []) ?(toplevel = false) ?expunge
  ...
  | `Pre magic -> (
    ...
    | `Cmo ->
        ...
        let a, b, c =
          from_compilation_units ~toplevel ~includes ~debug ~debug_data
            [compunit, code]
        in
        a, b, c, false
    | `Cma ->
          ...
          List.map
            (fun compunit ->
              ...
              then ()
                ...
                seek_in ic compunit.Cmo_format.cu_debug ;
                Debug.read_event_list debug_data ~crcs:[] ~includes ~orig:!orig
                  ic ) ;
