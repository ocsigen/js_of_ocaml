...
class map_for_share_constant =
  object (m)
    ...
    method expression e =
      match e with
      (* JavaScript engines recognize the pattern 'typeof x==="number"'; if the
         string is shared, less efficient code is generated. *)
      ...
      (* Some js bundler get confused when the argument of 'require' is not a
         litteral *)
    ...
    method private pack all sources =
      let may_flush rem vars s instr =
        ...
        then rem, [], s :: instr
          ...
          ( rem
          , []
          , s :: (Statement (Variable_statement (List.rev vars)), N) :: instr )
      in
      let rem, vars, instr =
        List.fold_left
...
let assign_op = function
  ...
  | ( exp
    , EBin
        (((Div | Mod | Lsl | Asr | Lsr | Band | Bxor | Bor) as unop), exp', y)
    )
    when exp = exp' ->
