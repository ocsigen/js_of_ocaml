...
struct
  ...
  module Solver (D : DOMAIN) = struct
    ...
    let f g f =
      ...
      let v =
        NSet.fold (fun x v -> incr n ; NMap.add x D.bot v) g.domain NMap.empty
      in
      ...
      (* let t3 = Util.Timer.get t3 in Format.eprintf "YYY %.2f %.2f %.2f@." t1
         t2 t3; Format.eprintf "YYY %d %d (%f)@." !m !n (float !m /. float !n); *)
      res
  end
end
