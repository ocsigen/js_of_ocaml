...
let options =
  ...
  let no_sourcemap =
    let doc =
      "Don't generate source map. All other source map related flags will be \
       be ignored."
    in
  ...
  let build_t common set_param set_env dynlink linkall toplevel export_file
      wrap_with_fun include_dir fs_files fs_output fs_external nocmis profile
      noruntime runtime_only no_sourcemap sourcemap sourcemap_inline_in_js
      sourcemap_don't_inline_content sourcemap_root output_file input_file
      js_files =
...
let info =
  ...
  let man =
    ...
    ; `P
        "js_of_ocaml is free software, you can redistribute it and/or modify \
