...
let error x y z r c =
  Printf.fprintf stdout
                 ...
                 x y z c (string_of_fpclass (Float.classify_float c))
                 (List.hd r)
...
let%expect_test _ =
  ...
  fma_test 0x4.000008p-128 ~-.0x4.000008p-28 ~-.0x8p+100 [~-.0x8p+100];
  [%expect {| OK! |}];
  fma_test 0x2.fep+12 0x1.0000000000001p+0 0x1.ffep-48 [0x2.fe00000000002p+12; 0x1.7f00000000002p+13];
  [%expect {| OK! |}];
  ...
  [%expect {| OK! |}];
  fma_test 0xe.f56df7797f768p+1020 0x3.7ab6fbbcbfbb4p-1024 ~-.0x3.40bf1803497f6p+0 [0x8.4c4b43de4ed2p-56; 0x1.095f287bc9da4p-53; 0x1.098p-53];
  [%expect {| OK! |}];
  fma_test 0x1.deadbeef2feedp+900 0x3.7ab6fbbcbfbb4p-1024 ~-.0x6.817e300692fecp-124 [0x1.0989687bc9da4p-176; 0x1.095f287bc9da4p-176; 0x1.098p-176];
  [%expect {| OK! |}];
  fma_test 0xf.ffffffffffff8p+1020 0x1.001p+0 ~-.0xf.ffffffffffff8p+1020 [0xf.ffffffffffff8p+1008; 0x1p+1012];
  [%expect {| OK! |}];
  fma_test ~-.0xf.ffffffffffff8p+1020 0x1.fffffffffffffp+0 0xf.ffffffffffff8p+1020 [~-.0xf.fffffffffffe8p+1020];
  [%expect {| OK! |}];
  fma_test 0xf.ffffffffffff8p+1020 0x2p+0 ~-.0xf.ffffffffffff8p+1020 [0xf.ffffffffffff8p+1020];
  [%expect {| OK! |}];
  fma_test 0x5.a827999fcef3p-540 0x5.a827999fcef3p-540 0x0p+0 [0x0p+0];
  [%expect {| OK! |}];
  fma_test 0x3.bd5b7dde5fddap-496 0x3.bd5b7dde5fddap-496 ~-.0xd.fc352bc352bap-992 [0x1.0989687cp-1044; 0x0.000004277ca1fp-1022; 0x0.00000428p-1022];
  [%expect {| OK! |}];
  fma_test 0x3.bd5b7dde5fddap-504 0x3.bd5b7dde5fddap-504 ~-.0xd.fc352bc352bap-1008 [0x1.0988p-1060; 0x0.0000000004278p-1022; 0x0.000000000428p-1022];
  [%expect {| OK! |}];
  fma_test 0x8p-540 0x4p-540 0x4p-1076 [0x8p-1076];
  [%expect {| OK! |}];
  fma_test 0x1.7fffff8p-968 0x4p-108 0x4p-1048 [0x4.0000004p-1048; 0x0.0000010000002p-1022];
  [%expect {| OK! |}];
  fma_test 0x2.8000008p-968 0x4p-108 0x4p-1048 [0x4.000000cp-1048; 0x0.0000010000002p-1022];
  [%expect {| OK! |}];
  fma_test 0x2.8p-968 ~-.0x4p-108 ~-.0x4p-1048 [~-.0x4.0000008p-1048];
  [%expect {| OK! |}];
  fma_test ~-.0x2.33956cdae7c2ep-960 0x3.8e211518bfea2p-108 ~-.0x2.02c2b59766d9p-1024 [~-.0x2.02c2b59767564p-1024];
  [%expect {| OK! |}];
  fma_test ~-.0x3.a5d5dadd1d3a6p-980 ~-.0x2.9c0cd8c5593bap-64 ~-.0x2.49179ac00d15p-1024 [~-.0x2.491702717ed74p-1024];
  [%expect {| OK! |}];
  fma_test 0x2.2a7aca1773e0cp-908 0x9.6809186a42038p-128 ~-.0x2.c9e356b3f0fp-1024 [~-.0x2.c89d5c48eefa4p-1024; ~-.0x0.b22757123bbe8p-1022];
  [%expect {| OK! |}];
  fma_test ~-.0x3.ffffffffffffep-712 0x3.ffffffffffffep-276 0x3.fffffc0000ffep-984 [0x2.fffffc0000ffep-984; 0x1.7ffffe00008p-983];
  [%expect {| OK! |}];
  ...
  fma_test 0x4p-1076 ~-.0x4p-1076 ~-.0x4p-1076 [~-.0x4p-1076];
  [%expect {| OK! |}];
  fma_test 0xf.ffffffffffff8p-4 0xf.ffffffffffff8p-4 ~-.0xf.ffffffffffffp-4 [0x4p-108; 0x0p+0];
  [%expect {| OK! |}];
  fma_test 0xf.ffffffffffff8p-4 ~-.0xf.ffffffffffff8p-4 0xf.ffffffffffffp-4 [~-.0x4p-108; 0x0p+0];
  [%expect {| OK! |}];
  fma_test ~-.0xf.ffffffffffff8p-4 0xf.ffffffffffff8p-4 0xf.ffffffffffffp-4 [~-.0x4p-108; 0x0p+0];
  ...
  fma_test 0x7.fffff8p-128 ~-.0x3.fffffcp+24 ~-.0xf.fffffp+124 [~-.0xf.fffffp+124];
  [%expect {| OK! |}];
  fma_test 0x7.ffffffffffffcp-1024 0x7.ffffffffffffcp+52 0xf.ffffffffffff8p+1020 [0xf.ffffffffffff8p+1020];
  [%expect {| OK! |}];
  fma_test 0x7.ffffffffffffcp-1024 ~-.0x7.ffffffffffffcp+52 0xf.ffffffffffff8p+1020 [0xf.ffffffffffff8p+1020];
  [%expect {| OK! |}];
  fma_test 0x7.ffffffffffffcp-1024 0x7.ffffffffffffcp+52 ~-.0xf.ffffffffffff8p+1020 [~-.0xf.ffffffffffff8p+1020];
