...
let%expect_test "track constraints record can be populated and read back" =
  let c = MediaCapture.empty_track_constraints () in
  c##.facingMode := Js.def (Js.string "user");
  c##.width := Js.def 1280;
  c##.echoCancellation := Js.def Js._true;
  let show f v = Js.Optdef.case v (fun () -> "undefined") f in
  print_endline (show Js.to_string c##.facingMode);
  print_endline (show string_of_int c##.width);
  print_endline (show (fun b -> string_of_bool (Js.to_bool b)) c##.echoCancellation);
  (* A field that was never constrained reads as [undefined]. *)
  print_endline (show string_of_int c##.height);
  [%expect {|
    user
    1280
    true
...
let%expect_test "stream constraints: plain and per-track forms share a field" =
  ...
  tc##.facingMode := Js.def (Js.string "environment");
  (* [video_constraints] must write the underlying [video] field. *)
  c##.video_constraints := tc;
