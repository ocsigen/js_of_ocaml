...
let console =
  make
    ...
    ~output:(fun section level logs ->
      let str =
        Js.string
          (Printf.sprintf "[%s] %s" (Section.name section)
             (String.concat "\n" logs))
      in
...
let log ?inspect ?exn ?section ?location ?logger ~level message =
  let inspect =
    match inspect with None -> None | Some v -> Some (Obj.repr v)
  in
...
let ign_log ?inspect ?exn ?section ?location ?logger ~level message =
  try ignore (log ?inspect ?exn ?section ?location ?logger ~level message)
  with _ -> ()

let ign_log_f ?inspect ?exn ?section ?location ?logger ~level format =
  Printf.ksprintf
    (ign_log ?inspect ?exn ?section ?location ?logger ~level)
    format
...
(*let raise_error ?inspect ?exn ?section ?location ?logger msg =
  Lwt.ignore_result (log ?inspect ?exn ?section ?location ?logger ~level:Error
  msg); failwith msg *)
