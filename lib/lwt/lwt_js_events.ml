  ...
  el :=
    Js.some
      (Dom.addEventListener target event_kind
         ...
         (* true because we do not want to prevent default -> the user can use
            the preventDefault function above. *)
         (Js.bool use_capture)) ;
  t
...
let seq_loop evh ?(cancel_handler = false) ?use_capture target handler =
  ...
  let rec aux () =
    if not !cancelled
       (* In the case it has been cancelled during the previous handler, we do
          not reinstall the event handler *)
    then (
