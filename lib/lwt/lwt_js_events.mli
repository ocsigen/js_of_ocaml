...
(** Reminder: Event capturing starts with the outer most element in the DOM and
    ...
    {[let rec esc elt = let%lwt ev = keydown elt in if ev##.keyCode = 27 then
    Lwt.return ev else esc elt]}

    Waiting for a click or escape key before continuing:

    {[let%lwt () = Lwt.pick [(let%lwt _ = esc Dom_html.document in Lwt.return
    ()); (let%lwt _ = click Dom_html.document in Lwt.return ())] in ...]}

    {2 Create Lwt threads for events} *)

(** [make_event ev target] creates a Lwt thread that waits for the event [ev]
    to happen on [target] (once). This thread is cancellable using {% <<a_api
    project="lwt" | val Lwt.cancel>> %}. If you set the optional parameter
    [~use_capture:true], the event will be caught during the capture phase,
    otherwise it is caught during the bubbling phase (default). *)
val make_event :
...
(** [seq_loop (make_event ev) target handler] creates a looping Lwt thread that
...
(** [buffered_loop] is similar to [seq_loop], but any event that occurs during
    an execution of the handler is queued instead of being ingnored.

    No event is thus missed, but there can be a non predictible delay between
    its trigger and its treatment. It is thus a good idea to use this loop with
    handlers whose running time is short, so the memorized event still makes
    sense when the handler is eventually executed. It is also up to the
    programmer to ensure that event handlers terminate so the queue will
    eventually be emptied.

    By default, cancelling the loop will not cancel the (potential) currently
    running handler, but any other queued event will be dropped. This behaviour
    can be customized using the two optional parameters [cancel_handler] and
    [cancel_queue]. *)
val buffered_loop :
     (?use_capture:bool -> 'target -> 'event Lwt.t)
  -> ?cancel_handler:bool
...
(** [async t] records a thread to be executed later. It is implemented by
    calling yield, then Lwt.async. This is useful if you want to create a new
    event listener when you are inside an event handler. This avoids the
...
(** [func_limited_loop event delay_fun target handler] will behave like
    ...
    first, and execut [handler] only when [delay_fun] is finished and no other
...
val load :
  ?use_capture:bool -> #Dom_html.imageElement Js.t -> Dom_html.event Js.t Lwt.t
