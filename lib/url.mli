...
(** [urlencode ?with_plus s] replace characters for their percent encoding
    ...
    [with_plus] is [true] (default) then ['+']'s are escaped as ["%2B"]. If
    not, ['+']'s are left as is. *)
...
module Current : sig
  ...
  (** [get ()] returns a value of type {!url} with fields reflecting the state
      of the current Url. *)
  ...
  (** [as_string] is the original string representation of the current Url. It
      is NOT necessarily equals to [string_of_url (get ())] but [url_of_string
      as_string = get ()] holds. *)
