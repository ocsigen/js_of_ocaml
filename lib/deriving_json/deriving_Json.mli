...
(** [convert (t : 'a t) (from_ : 'a -> 'b) (to_ : 'b -> 'a)] generate a JSON
    parser/printer for value of type ['b] using the parser/printer of type ['a]
    provided by [t] *)
...
(** Parse and serialize a map as if it was an array of tuple. {[ (* My map
    ...
    A.a t type b = A.a StringMap.t let t = S.t let to_ : b -> A.a t = fun a ->
    Array.of_list (StringMap.bindings a) let from_ : A.a t -> b = fun l ->
    Array.fold_left (fun map (x,v) -> StringMap.add x v map) StringMap.empty l
    end) end ]}
