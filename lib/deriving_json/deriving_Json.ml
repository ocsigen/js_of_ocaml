	...
	in aux xs 0
 	...
 	let rec aux l c =
	  match Deriving_Json_lexer.read_case buf with
		...
		Deriving_Json_lexer.read_comma buf;
		aux (x::l) (succ c)
	    | _ -> Deriving_Json_lexer.tag_error ~classname:"list" buf
	in
	aux [] 0
    end)
...
module Json_ref(A : Json) = Defaults(struct
	...
	| `NCst 0 ->
	    ...
	    Deriving_Json_lexer.read_rbracket buf;
	    ref x
	| _ -> Deriving_Json_lexer.tag_error ~classname:"ref" buf
    end)
...
module Json_option(A : Json) = Defaults(struct
	...
	| `NCst 0 ->
	    ...
	    Deriving_Json_lexer.read_rbracket buf;
	    Some x
	| _ -> Deriving_Json_lexer.tag_error ~classname:"option" buf
    end)
...
module Json_array(A : Json) = Defaults(struct
	...
	| _ -> Deriving_Json_lexer.tag_error ~classname:"array" buf
    end)
