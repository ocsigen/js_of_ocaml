  ...
  type t =
    ...
    | RGBA of (int * int * int * float)
        (**Same as RGB with additionnal transparency argument. Opacity should
           be in [0.] (completely transparent) and [1.] (completely opaque).*)
    ...
    | HSL of (int * int * int)
        (**Hue, Saturation and Lightness values. Hue is an angle in degree (in
           interval [[0..360[]). Saturation is a percentage ([[0..100]]) with
           [0] being colorless. Lightness is also a percentage ([[0..100]])
           with [0] being black.*)
      ...
      let i_of_s_o = function
        ...
        | Some i -> (
          ...
          | Invalid_argument s | Failure s ->
              raise
                (Invalid_argument ("color conversion error (" ^ i ^ "): " ^ s))
          )
      in
      let f_of_s f =
