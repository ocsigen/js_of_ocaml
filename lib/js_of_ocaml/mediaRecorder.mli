...
(** MediaStream Recording: record a {!MediaCapture.class-type-mediaStream} into
    {!File.blob} chunks.
...
class type ['a] mediaRecorderErrorEvent = object
  inherit ['a] Dom.event
...
class type mediaRecorder = object ('self)
  ...
  (* [audioBitrateMode] (["constant"] or ["variable"]) is deliberately not
     bound: it is still experimental and not Baseline, so reading it yields
  ...
  method ondataavailable : ('self t, 'self blobEvent t) Dom.event_listener writeonly_prop

  method onstart : ('self t, 'self Dom.event t) Dom.event_listener writeonly_prop

  method onstop : ('self t, 'self Dom.event t) Dom.event_listener writeonly_prop

  method onpause : ('self t, 'self Dom.event t) Dom.event_listener writeonly_prop

  method onresume : ('self t, 'self Dom.event t) Dom.event_listener writeonly_prop

  method onerror :
    ('self t, 'self mediaRecorderErrorEvent t) Dom.event_listener writeonly_prop
end
