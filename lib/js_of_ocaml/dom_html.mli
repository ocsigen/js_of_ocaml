...
class type mediaError = object
  ...
  (** 1 = aborted, 2 = network error, 3 = decoding error, 4 = source not
      supported. *)

  method message : js_string t readonly_prop
end

(** {2 Text tracks (subtitles, captions, chapters, metadata)} *)
...
class type textTrackCue = object ('self)
  ...
  method onenter : ('self t, 'self Dom.event t) event_listener writeonly_prop

  method onexit : ('self t, 'self Dom.event t) event_listener writeonly_prop
end

(** Access the cues by index with [Js.array_get (Js.Unsafe.coerce list)]. *)
...
and textTrack = object ('self)
  ...
  (** One of ["subtitles"], ["captions"], ["descriptions"], ["chapters"] or
      ["metadata"]. *)
  ...
  (** One of ["disabled"], ["hidden"] or ["showing"]. *)
  ...
  method oncuechange : ('self t, 'self Dom.event t) event_listener writeonly_prop
end
...
and textTrackList = object ('self)
  ...
  method onchange : ('self t, 'self Dom.event t) event_listener writeonly_prop
...
and textTrackEvent = object
  inherit [textTrackList] Dom.event

  method track : textTrack t opt readonly_prop
end

(** A WebVTT cue: the concrete kind of {!class-type:textTrackCue} that scripts
    can create. The [line] and [position] settings, which are either a number
    or the string ["auto"], are not bound. *)
class type vttCue = object
  inherit textTrackCue

  method text : js_string t prop

  method vertical : js_string t prop
  (** One of [""] (horizontal), ["rl"] or ["lr"]. *)

  method snapToLines : bool t prop

  method size : number_t prop
  (** Percentage of the video width (or height when vertical). *)

  method align : js_string t prop
  (** One of ["start"], ["center"], ["end"], ["left"] or ["right"]. *)

  method lineAlign : js_string t prop

  method positionAlign : js_string t prop

  method getCueAsHTML : Dom.documentFragment t meth
end

val vttCue : (number_t -> number_t -> js_string t -> vttCue t) constr
(** [new%js vttCue startTime endTime text]; add it to a track with
    [track##addCue (cue :> textTrackCue Js.t)]. *)
...
class type mediaElement = object
  ...
  (** [el##addTextTrack_full kind label language]. *)
  ...
  (* [srcObject] is deliberately not bound: it is not Baseline. Only Safari
     accepts all of its value types ([MediaStream], [MediaSource], [Blob],
     [File]); the other browsers accept a [MediaStream] only, and [Dom_html]
     cannot depend on [MediaCapture] to type that case. Set it with
     [Js.Unsafe.set el (Js.string "srcObject") stream] for now. *)
  ...
  (** Fired when loading or decoding fails; inspect [el##.error] for the
      cause. *)
