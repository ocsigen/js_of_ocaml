...
(** A node in the audio graph. [connect] returns the destination node to allow
    ...
    The [_withOutput] / [_withOutputAndInput] variants select which output of
    this node and which input of the destination are wired, for nodes with
    several of them ({!class-type:channelSplitterNode},
    {!class-type:channelMergerNode}); the plain forms use output 0 and
    input 0. *)
class type audioNode = object
  ...
  method connect_withOutput : audioNode t -> int -> audioNode t meth

  method connect_withOutputAndInput : audioNode t -> int -> int -> audioNode t meth
  (** [n##connect_withOutputAndInput dest output input]. *)

  method connect_param : audioParam t -> unit meth
  (** Connects the node's output to an {!class-type:audioParam}, modulating it
      with the audio signal. *)

  method connect_param_withOutput : audioParam t -> int -> unit meth

  method disconnect : unit meth
  (** Disconnects every outgoing connection. *)

  method disconnect_output : int -> unit meth
  (** Disconnects every connection from the given output. *)

  method disconnect_node : audioNode t -> unit meth

  method disconnect_node_withOutput : audioNode t -> int -> unit meth

  method disconnect_node_withOutputAndInput : audioNode t -> int -> int -> unit meth

  method disconnect_param : audioParam t -> unit meth

  method disconnect_param_withOutput : audioParam t -> int -> unit meth
end
...
(** An output node whose audio is exposed as a {!MediaCapture.class-type-mediaStream}
