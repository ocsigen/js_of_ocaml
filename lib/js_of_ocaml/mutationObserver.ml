...
class type mutationObserverInit =
  object
    method childList : bool Js.t Js.writeonly_prop

    method attributes : bool Js.t Js.writeonly_prop

    method characterData : bool Js.t Js.writeonly_prop

    method subtree : bool Js.t Js.writeonly_prop

    method attributeOldValue : bool Js.t Js.writeonly_prop

    method characterDataOldValue : bool Js.t Js.writeonly_prop
...
let observe
  ...
  let () = opt_iter child_list (fun v -> cfg##.childList := Js.bool v) in
  let () = opt_iter attributes (fun v -> cfg##.attributes := Js.bool v) in
  let () = opt_iter character_data (fun v -> cfg##.characterData := Js.bool v) in
  let () = opt_iter subtree (fun v -> cfg##.subtree := Js.bool v) in
  let () = opt_iter attribute_old_value (fun v -> cfg##.attributeOldValue := Js.bool v) in
  let () =
    opt_iter character_data_old_value (fun v -> cfg##.characterDataOldValue := Js.bool v)
  in
  let () =
