...
class type mediaRecorder = object ('self)
  ...
  (* [audioBitrateMode] is not bound: not Baseline, see the [.mli]. *)
  ...
  method ondataavailable :
    ('self Js.t, 'self blobEvent Js.t) Dom.event_listener Js.writeonly_prop

  method onstart : ('self Js.t, 'self Dom.event Js.t) Dom.event_listener Js.writeonly_prop

  method onstop : ('self Js.t, 'self Dom.event Js.t) Dom.event_listener Js.writeonly_prop

  method onpause : ('self Js.t, 'self Dom.event Js.t) Dom.event_listener Js.writeonly_prop

  method onresume :
    ('self Js.t, 'self Dom.event Js.t) Dom.event_listener Js.writeonly_prop

  method onerror :
    ('self Js.t, 'self mediaRecorderErrorEvent Js.t) Dom.event_listener Js.writeonly_prop
end
