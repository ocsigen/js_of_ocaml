...
(** Bindings to the JavaScript [Promise] API.
    ...
    These bindings work around that by wrapping every value in a small
    container before resolving and unwrapping it on the way out. As a
    result, ['a t] values always resolve with a value of OCaml type ['a],
    even when ['a] is itself ['_ t].
