...
(** Media Capture and Streams: [MediaStream], [MediaStreamTrack] and
    ...
    {!Dom_html.mediaElement} (not bound, as it is not Baseline):
    {[
...
(** Constraints on a single track. All fields are optional: create an empty
    record with {!empty_track_constraints} and populate the ones you need with
    {!Js.def}. The same record is returned by [track##getConstraints], where
    the fields that were not constrained read as [undefined].
    ...
    {!Js.Unsafe.set} (such a value then reads back at the plain type; use
    {!Js.Unsafe.get} for it). *)
class type mediaTrackConstraints = object
  method deviceId : js_string t optdef prop

  method groupId : js_string t optdef prop

  method facingMode : js_string t optdef prop

  method width : int optdef prop

  method height : int optdef prop

  method aspectRatio : number_t optdef prop

  method frameRate : number_t optdef prop

  method resizeMode : js_string t optdef prop

  method sampleRate : int optdef prop

  method sampleSize : int optdef prop

  method channelCount : int optdef prop

  method echoCancellation : bool t optdef prop

  method noiseSuppression : bool t optdef prop

  method autoGainControl : bool t optdef prop
end
...
(** Argument of [getUserMedia]/[getDisplayMedia]: which kinds of tracks are
    ...
    [video]) or requested with per-track constraints ([audio_constraints],
    [video_constraints]; each writes the same field as its boolean form).
    Create an empty record with {!empty_stream_constraints}. *)
class type mediaStreamConstraints = object
  method audio : bool t writeonly_prop

  method audio_constraints : mediaTrackConstraints t writeonly_prop

  method video : bool t writeonly_prop

  method video_constraints : mediaTrackConstraints t writeonly_prop
end
...
class type ['a] trackEvent = object
  inherit ['a] Dom.event
...
class type mediaStream = object ('self)
  ...
  method onaddtrack : ('self t, 'self trackEvent t) Dom.event_listener writeonly_prop

  method onremovetrack : ('self t, 'self trackEvent t) Dom.event_listener writeonly_prop
end
