...
let peak (buffer : WebAudio.audioBuffer Js.t) channel =
  let data = buffer##getChannelData channel in
  let peak = ref 0. in
...
(* Route a mono oscillator through a splitter into input 1 of a merger: only
   the right channel of the stereo rendering should carry signal. This is what
   the indexed [connect] overloads are for. *)
let test_channel_routing () =
  let ctx = new%js WebAudio.offlineAudioContext 2 4410 (num 44100.) in
  let osc = ctx##createOscillator in
  let splitter = ctx##createChannelSplitter_withCount 2 in
  let merger = ctx##createChannelMerger_withCount 2 in
  check
    "splitter and merger have two outputs / inputs"
    (splitter##.numberOfOutputs = 2 && merger##.numberOfInputs = 2)
    "";
  ignore (osc##connect (splitter :> WebAudio.audioNode Js.t) : WebAudio.audioNode Js.t);
  let dest =
    splitter##connect_withOutputAndInput (merger :> WebAudio.audioNode Js.t) 0 1
  in
  check
    "indexed connect returns the destination"
    (dest == (merger :> WebAudio.audioNode Js.t))
    "";
  ignore
    (merger##connect (ctx##.destination :> WebAudio.audioNode Js.t)
      : WebAudio.audioNode Js.t);
  osc##start;
  ctx##startRendering
  >>= fun buffer ->
  let left = peak buffer 0 and right = peak buffer 1 in
  check
    "signal reached only merger input 1"
    (left < 0.001 && right > 0.9)
    (Printf.sprintf "left=%.3f right=%.3f" left right);
  return ()
...
let test_media_element (stream : MediaCapture.mediaStream Js.t) =
  ...
  (* [srcObject] is not bound (not Baseline); set it as the MediaCapture
     documentation recommends. *)
  Js.Unsafe.set el (Js.string "srcObject") stream;
  check
    "srcObject round-trips"
    (Js.Unsafe.get el (Js.string "srcObject") == Js.Unsafe.inject stream)
    "";
  ...
  let cue = new%js Dom_html.vttCue (num 0.) (num 1.) (Js.string "hello") in
  track##addCue (cue :> Dom_html.textTrackCue Js.t);
  check
    "VTTCue can be added to the track"
    (Js.Opt.case track##.cues (fun () -> false) (fun cues -> cues##.length = 1)
    && Js.to_string cue##.text = "hello")
    (Js.to_string cue##.align);
...
let run () =
  ...
  let tests =
    test_offline_render () >>= test_channel_routing >>= test_realtime >>= test_devices
  in
  let _ : unit Promise.t =
    Promise.catch
      (fun e ->
